...
namespace QuickSoft.Controllers
{
	...
	public class TaskItemController : BaseController
	{
		...
		public ActionResult GetBOM(long? BName, long? Item, decimal? Qty, long? Unit)
		{
			...
			{
				v = v.Where(p => p.BOMName.ToString().ToLower().Contains(search.ToLower()) ||
		...
		public JsonResult SearchBOM(string q, string x)
		{
			...
			{
				serialisedJson = db.BillOfMaterials.Where(p => p.BOMName.ToLower().Contains(q.ToLower()) || p.BOMName.Contains(q))
			...
			{
				serialisedJson = db.BillOfMaterials.Select(b => new SelectFormat
				{
					text = b.BOMName, //each json object will have 
					id = b.BOMId
				}).OrderBy(b => b.text).ToList();
