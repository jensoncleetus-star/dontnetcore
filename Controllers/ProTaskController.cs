    ...
    public class ProTaskController : BaseController
    {
     ...
     (From == "" || EF.Functions.DateDiffDay(a.LogTime, datefrom) <= 0) &&
        ...
        public ActionResult cald(long? driver,string taskdate)
        {
            ...
            for (var i = 0; i < 38; i++)
            {
                ...
                foreach(var t in task)
                {
                    if (taskcount == 0)
                    {
                        string ass = assignedLookup.TryGetValue(t.ProTaskId, out var assNames) ? assNames : "";
        ...
        public ActionResult GetAllTaskdash(long protaskstatus)
        {
            ...
            // taskups = latest ProTaskUpdation per task. The original GroupBy->FirstOrDefault-of-ENTITY
            // LEFT JOIN is not translatable by EF Core 10; it is recomputed client-side below (taskupLookup).
            ...
            */
            ...
            // LEFT JOIN, the let Reminder/let mobnum subqueries and the nested AssignedTo/TskLead/
            // Updateduser collection projections are removed here and recomputed CLIENT-SIDE below.
            // (The Contacts join `i` was never projected in the original output, so it is dropped.)
            ...
            // latest ProTaskUpdation per task (was the taskups GroupBy-latest + taskup LEFT JOIN)
            ...
            // latest reminder per task (was the let Reminder subquery, ordered by RDate desc)
            ...
            // CLIENT re-projection — identical member names, order & values to the original outer Select.
            var UserView = rawRows.Select(r =>
            {
                ...
                DateTime? ldate = ((a != null) && (taskup != null && taskup.CreatedDate.HasValue && taskup.CreatedDate.Value > a.logtime)) ? taskup.CreatedDate : (DateTime?)a.logtime;
                return new
                {
                    a.ProTaskId,
                    a.TaskName,
...
union select taskid, salesentryid as saleid  from additionaltaks ) as q1)))").ToArray();
      ...
      public List<MobileViewModel> mobilesno(long protaskid)
        ...
        public ActionResult ServiceReport(ServiceReportViewModel vmodel)
        {
            ...
            var itemsused = (from a in db.itemtasklist
            ...
            var printdata = (hdr == null) ? null : new
            {
                hdr.servicereportid,
                hdr.TaskCode,
                hdr.TaskName,
        ...
        public StringBuilder generatepdfservicereport(long id)
        {
            ...
            var ptid = (hdr == null) ? 0 : hdr.ProTaskId;
            ...
            var itemsused = (from aa in db.itemtasklist
                             ...
                             where aa.protaskid == ptid && c.MCId == mymc
                             select new
                             {
                                 b.ItemCode,
                                 b.ItemName,
