...
namespace QuickSoft.Controllers
{
    ...
    public class PurchaseEntryController : BaseController
    {
        ...
        Common com;
        public PurchaseEntryController()
        {
        ...
        public ActionResult setopenclose(long id)
        {
            var Stat = QkSelect.List(
                ...
                new SelectListItem { Value="0",Text="Open"}, new SelectListItem { Value="1",Text="Close"},}, "Value", "Text");
        ...
        {
            var pr = db.PurchaseEntrys.Find(protaskid);
            pr.PurchaseStatus = OpenClose;
                                ...
                                if(System.IO.File.Exists(newName))
{
        ...
        public ActionResult GetSalesEntrys(string billno)
        {
            ...
            if (isemirtech)
                ...
                db = new ApplicationDbContext("emirtech");
        ...
        public JsonResult CreatePurchase(string[][] array, string[] purchasedata, PEBillSundryViewModel bsmodel, ICollection<BatchStockPViewModel> bstmodel, ICollection<RackStockPViewModel> bsrackData)
        {
            ...
            var mrnos = Convert.ToString(purchasedata[45]);
            ...
            if (!exist)
            {
                ...
                com.ItemTransInCreateMode("Purchase", MC, 0, 0, array, UserId, Today);
                ...
                foreach (var arr in array)
                {
                    var arrpre = arr[26];
                    var ItemCd = arr[14];
                    var itemname = Convert.ToString(arr[13]);
                    var itemid = Convert.ToInt64(arr[0]);
                    Int64 retval = 0;
                    var Cdchk = db.Items.Where(y => y.ItemCode == ItemCd).Any();
                    var Codechk = (arr[0] != "0") || Cdchk;
                    if (Codechk == false)
                    {
                        ItemViewModel itemz = new ItemViewModel();
                        var pre = db.PrefixMasters.Where(y => y.PrefixCode == arrpre).Select(x => x.PrefixCode).FirstOrDefault();
                        var category = Convert.ToString(arr[15]);
                        var brand = Convert.ToString(arr[16]);
                        var pprice = Convert.ToDecimal(arr[17]);
                        var newCategory = (arr[15] == null || arr[15] == "") ? 1 : com.GetCategoryId(category);
                        var newbrand = (arr[16] == null || arr[16] == "") ? 1 : com.GetBrandId(brand);
                        var master = db.PrefixMasters.Where(x => x.PrefixCode == arrpre).Any();
                        string newprefix = "";
                        if (PreChk == Status.active)
                        {
                            newprefix = PrefixCode(arrpre);
                            itemz.ItemCode = newprefix;
                            itemz.Barcode = newprefix;
                            itemz.Prefix = (arr[26] == "") ? 0 : GetPrefixId(arr[26], newbrand, newCategory);
                        }
                        else
                        {
                            var code = ItemCodes();
                        ...
                        long TaxId = (Convert.ToInt32(arr[10]) == 0) ? 0 : com.GetTaxId(Convert.ToDecimal(arr[10]));


                        itemz.ItemName = itemname;
                        itemz.ItemCategoryID = newCategory;
                        itemz.ItemBrandID = newbrand;
                        itemz.Supplier = Convert.ToInt64(purchasedata[0]);
                        itemz.SupplierRef = arr[28];
                        itemz.PurchasePrice = pprice;
                        itemz.MRP = Convert.ToDecimal(arr[18]);
                        itemz.SellingPrice = Convert.ToDecimal(arr[19]);
                        itemz.BasePrice = Convert.ToDecimal(arr[20]);
                        //time being tax direct given //2 = 5% tax and 1 = 0 tax
                        itemz.TaxID = TaxId;
                        itemz.Branch = Convert.ToInt32(BranchID);
                        itemz.ItemType = ItemType(arr[27]);
                        itemz.ConFactor = 1;
                        if (EnableJew == Status.active)
                        {
                            itemz.Tagline1 = arr[21];
                            itemz.Tagline2 = arr[22];
                            itemz.Tagline3 = arr[23];
                            itemz.Tagline4 = arr[24];
                            itemz.Tagline5 = arr[25];
                        }
                        ...
                        {
                            retval = InsertDummyPEItems(arr, purchaseEntryId, id, Convert.ToString(purchasedata[14]));
                        
                    }
                        else
                        {
                            retval = InsertPEItems(arr, purchaseEntryId, id, Convert.ToString(purchasedata[14]));
                        }
                    }
                    else
                    {
                            var Item = (arr[0] == "0") ? db.Items.Where(x => x.ItemCode == arr[14]).Select(y => y.ItemID).FirstOrDefault() : itemid;
                        if (brcheck == Status.active)
                        {
                            retval = InsertDummyPEItems(arr, purchaseEntryId, Item, Convert.ToString(purchasedata[14]));

                        }
                        else
                        {
                            retval = InsertPEItems(arr, purchaseEntryId, Item, Convert.ToString(purchasedata[14]));
                        }
                        }
                ...
                {
                    var stocktanferid = db.StockTransfers.Where(o => o.Voucher == "Material Recieve Note , Voucher no:" + CMRNoteNo).Select(o => o.Id).FirstOrDefault();
        ...
        public ActionResult GetPurchaseEntryPaymentApproved(string BillNo, string FromDate, string ToDate, long? supplier, long? salesperson, long? type, string user, int? Balance, long? MC, string appstat, string PurchaseType, long? HireType, long? PurchaseStatus, string RefenceNo)
        {
            ...
            // `AppStatus`, `chkAppStatus`, `chkAppStatusme` lets and the `sh` ConvertTransactionss subquery).
            // Split SERVER from CLIENT: materialize only entity columns + simple scalars (left-joined entity
            // access like Supplier/EmpName/MC stays server-side) into serverRows, then build client lookups
            // keyed by PurchaseEntryId and re-project client-side with the SAME member names + order.
            var serverRows = (from a in db.PurchaseEntrys
                     ...
                     from h in hir.DefaultIfEmpty()
                         ...
                         // app/AppStatus/chkAppStatus/chkAppStatusme/sh are computed client-side after
                         // materialization — EF Core 10 can't translate them inside this query.
            ...
            // app = approver EmployeeIds (constant list: AssignedTos for CustomerID == -1, same for every row).
            var app = db.AssignedTos.Where(x => x.CustomerID == -1).Select(x => x.EmployeeId).ToList();
            // AppStatus = all ApprovalUpdate statuses; raw rows materialized once and reused for chkAppStatus/meexist.
            var appUpdRows = db.ApprovalUpdates
                .Where(x => x.Type == "PurchaseEntryPayment" && peIds.Contains(x.TransEntry))
            ...
            // meexist = ApprovedBy of my own Approved ApprovalUpdate (FirstOrDefault), per purchase entry.
            var meexistLookup = appUpdRows
                .Where(x => x.ApprovalStatus == ApprovalStatus.Approved && x.ApprovedBy == UserId)
                .GroupBy(x => x.TransEntry)
                .ToDictionary(g => g.Key, g => g.Select(x => x.ApprovedBy).FirstOrDefault());
            // sh = HExtent: ConvertTransactionss row (PurchaseExtend -> Purchase) per purchase entry.
            var shLookup = db.ConvertTransactionss
                .Where(ap => ap.ConvertFrom == tov && ap.ConvertTo == fromv && peIds.Contains(ap.From))
                .Select(ap => new { ap.From, ap.ConvertFrom })
                .ToList()
                .GroupBy(ap => ap.From)
                .ToDictionary(g => g.Key, g => g.Select(ap => ap.ConvertFrom).FirstOrDefault());
            ...
            var v = serverRows.Select(o =>
                     {
                         ...
                         var meexist = meexistLookup.TryGetValue(o.PurchaseEntryId, out var me) ? me : null;
                         var HExtent = shLookup.TryGetValue(o.PurchaseEntryId, out var sh) ? sh : null;
                         return new
                     {
                         o.PurchaseEntryId,
                         o.PECreatedDate,
        ...
        public ActionResult GetPurchaseEntry(string BillNo, string FromDate, string ToDate, long? supplier, long? salesperson, long? type, string user, int? Balance, long? MC, string appstat, string PurchaseType, long? HireType, long? PurchaseStatus, string RefenceNo)
        {
            ...
            var serverQuery = (from a in db.PurchaseEntrys
                   ...
                   //  let qs = db.ConvertTransactionss.Where(ap => ap.From == a.PurchaseEntryId && ap.ConvertFrom == fromv && ap.ConvertTo == Tosales).FirstOrDefault()
                     ...
                     select new
                     {
                         validornot = tem != 1 && ( EF.Functions.DateDiffMinute(a.PEDate, editableDay) <= 0 && EF.Functions.DateDiffMinute(a.PEDate , today) >= 0) ? "valid" :(a.PEDate > today)? "valid":"invalid",
                         userEditDays = userEditDays,
                         SaleConvert =0,// qs.ConvertTo,
                         a.PurchaseEntryId,
                         a.PENo,
                         tax = j.Debit == null ? 0 : j.Debit ,
                         a.BillNo,
                         a.PEDate,
                         a.PECreatedDate,
                         a.PEGrandTotal,
            ...
            {
                if (isranan == true && sortColumn == "ReferenceNo")
                {
                    v = v.OrderByDescending(o=>long.Parse(o.ReferenceNo));// + " " + sortColumnDir);
                }
                else
                {
        ...
        public ActionResult Edit(long? id)
        {
            ...
            var UserId = User.Identity.GetUserId();
            PurchaseEntry PEentry = db.PurchaseEntrys.Where(x => (x.CreatedBy == UserId || userpermission == true) && x.PurchaseEntryId == id).FirstOrDefault();
           if(PEentry.PEItemQuantity==0)
            {
                return RedirectToAction("Edit/" + id, "PurchaseEntryExpense");
            }
            var today = DateTime.Now;
            var editableDay = DateTime.Now;
            var tem = 0;
        ...
        public ActionResult UpdatePurchase(string[][] array, string[] purchasedata, PEBillSundryViewModel bsmodel, ICollection<BatchStockPViewModel> bstmodel, ICollection<RackStockPViewModel> bsrackData)
        {
            ...
            if (com.islocked("Purchase", PEentry.PEDate))
            {
            ...
            if (1==1)
            {
                ...
                com.ItemTransInEditMode("Purchase", MC, 0, 0, array, purchaseEntryId, UserId, CurrentDate);
        ...
        public ActionResult AddAmcRemark(int? id)
        {
            ...
            PurchaseEntry cus = db.PurchaseEntrys.Find(id);
            ...
            {
                TransactionId = cus.PurchaseEntryId,
                TransactionType = "PurchaseRemark"
        ...
        public JsonResult AddRemark(AddedRemarksvm amcremarks)
        {
            ...
            if (ModelState.IsValid)
            {
                if (amcremarks.Remarks != null)
                {
                    ...
                    com.addlog(LogTypes.Created, UserId, "purchaseremark", "AddedRemarks", findip(), amcid, "AMC Remarks Added Successfully..");
        ...
        public JsonResult SearchPurchaseEntry(string q, string page, int ptype,long? supplier)
        {
            ...
            if (!(string.IsNullOrEmpty(q) && string.IsNullOrEmpty(q)))
            {
                if (ptype == 0)
                {
                    ...
                    serialisedJson = db.PurchaseEntrys.Where(o => (supplier == null || supplier == 0||o.Supplier==supplier)).Where(p => p.PurType == PurchaseHireType.Purchase && p.Status == 1 && (p.BillNo.ToLower().StartsWith(q.ToLower()) || p.BillNo.StartsWith(q)))
                ...
                {
                    serialisedJson = db.PurchaseEntrys.Where(o => (supplier == null || supplier == 0 || o.Supplier == supplier)).Where(p => p.PurType == PurchaseHireType.CrossHire && p.Status == 1 && (p.BillNo.ToLower().StartsWith(q.ToLower()) || p.BillNo.StartsWith(q)))
                           .Select(b => new SelectFormat
                           {
                               text = b.BillNo, //each json object will have 
                               id = b.PurchaseEntryId
            ...
            else
            {
                if (ptype == 0)
                {
                    serialisedJson = db.PurchaseEntrys.Where(o => (supplier == null || supplier == 0 || o.Supplier == supplier)).Where(p => p.PurType == PurchaseHireType.Purchase && p.Status == 1).Select(b => new SelectFormat
                    {
                        text = b.BillNo, //each json object will have 
                        id = b.PurchaseEntryId
                    }).OrderBy(b => b.text).Skip(skip).Take(pageSize).ToList();
                ...
                {
                    serialisedJson = db.PurchaseEntrys.Where(o => (supplier == null || supplier == 0 || o.Supplier == supplier)).Where(p => p.PurType == PurchaseHireType.CrossHire && p.Status == 1).Select(b => new SelectFormat
                    {
                        text = b.BillNo, //each json object will have 
                        id = b.PurchaseEntryId
                    }).OrderBy(b => b.text).Skip(skip).Take(pageSize).ToList();
        ...
        public ActionResult EditStatus(ApprovalUpdate App, long id)
        {
            ...
            if (brcheck == Status.active)
            {
                if (App.ApprovalStatus == ApprovalStatus.Approved)
                {
                    ...
                    {
                        var exist = db.PEItemss.Any(o => o.Item == arr.Item && o.PurchaseEntry == arr.PurchaseEntry && o.ItemQuantity == arr.ItemQuantity);
                        if (!exist)
                        {
                            //add to dummy table
        ...
        public ActionResult GetAllRemarksadded(long? RequisitionId)
        {
            ...
            var v = (from a in db.AddedRemarks
                     ...
                     where a.TransactionId == RequisitionId && a.TransactionType == "PurchaseRemark" && a.Remarks != null
        ...
        public ActionResult GetLastPurchase(int? purchasecount, long? ItemId,long? mc)
        {
            ...
            var v = (from b in db.PEItemss
                     ...
                     {
                         b.PEItemsId,
                         // s is left-joined (DefaultIfEmpty) -> cast non-nullable struct columns to nullable so
                         // a NULL row does not throw "Nullable object must have a value" on materialization.
                         PEDate = (DateTime?)s.PEDate,
                         PurchaseEntryId = (long?)s.PurchaseEntryId,
                         s.BillNo,
                         mc=m.MCName,
                         p.SupplierName,
                         ItemUnitPrice = b.ItemUnitPrice,
        ...
        {          
            ...
            if (ItemId != null)
            {
                ...
                var BatchStock = (from a in db.BatchStocks
                                  ...
                                  {
                                      SerialNo      =   a.BatchNo,
                                      ExpiryDate    =   (a.EXP != null) ? a.EXP : (DateTime?)c.PEDate.AddDays((double)((d.daysexpirty != null)? d.daysexpirty.Value : 360)),
        ...
        public JsonResult GetSerialNoDtlInCreateMode(long PurchEntryId, long ItemId)
        {
            var ConD = (from a in db.Items
                        ...
                        select new
                        {
                            ...
                            ExpiryDate  =   (DateTime?)c.PEDate.AddDays((double)((a.daysexpirty != null)? a.daysexpirty.Value : 360)),
                            UnitId      =   b.ItemUnit
                        }).ToList();
            if(ConD==null||ConD.Count()==0)
            {
                 ConD = (from a in db.Items
                            join b in db.DummyPEItems2
                            on a.ItemID equals b.Item
                            join c in db.PurchaseEntrys
                            on b.PurchaseEntry equals c.PurchaseEntryId
                            where (c.PurchaseEntryId == PurchEntryId && a.ItemID == ItemId)
        ...
        public bool createitem(long itemid)
        {
            ApplicationDbContext db = new ApplicationDbContext("quicknet");
            ItemViewModel itvmodel = new ItemViewModel();
            var item = db.Items.Find(itemid);
            itvmodel.ItemCode = item.ItemCode;
            itvmodel.KeepStock = item.KeepStock;
            itvmodel.ItemName = item.ItemCode;
            itvmodel.ItemDescription = item.ItemDescription;
            itvmodel.SubUnitId = item.SubUnitId;
            itvmodel.ItemUnitID = item.ItemUnitID;
            itvmodel.ConFactor = item.ConFactor;
            itvmodel.SellingPrice = item.SellingPrice;
            itvmodel.PurchasePrice = item.PurchasePrice;
