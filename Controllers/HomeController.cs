    ...
    public class HomeController : BaseController
    {
        ...
        public ActionResult Index()
        {
            ...
            if (UserId != null)
            {
                ...
                else if (Request.Cookies["QuickERP2"] != hash.PasswordHash.ToString())
                {
                }
            }
        ...
        //  [HttpPost]
        public ActionResult GetPurchaseEntry()
        {
            var UserId = User.Identity.GetUserId();
            ...
            var v = (from a in db.PurchaseEntrys
                     ...
                     select new
                     {
                         ...
                         a.PEGrandTotal,
                         c.PEPaidAmount,
                         PEBalanceAmount = a.PEGrandTotal - c.PEPaidAmount
                     });
            v = v.OrderByDescending(c => c.PurchaseEntryId).Take(5);
            var data = v.ToList();
        ...
        public JsonResult GetPDC()
        {
            ...
            var v = (from a in db.PDCs
                     join b in db.Payments on a.Reference equals b.PaymentId into pay
                     from b in pay.DefaultIfEmpty()
                     join c in db.Receipts on a.Reference equals c.ReceiptId into rec
                     ...
                     where (a.RegStatus == choice.No) &&
                     ((a.PDCType == "Payment" && a.Reference == b.PaymentId) ||
                     (a.PDCType == "Receipt" && a.Reference == c.ReceiptId))
                      && (a.PDCDate <= newdate)
                     select new
                     {
                         id = a.PDCid,
                         Date = a.PDCDate,
                         Voucher = (b.VoucherNo != null) ? b.VoucherNo : c.VoucherNo,
                         Account = (d.Name != null) ? d.Name : e.Name,
                         Issued = (decimal?)b.GrandTotal,
                         Receipt = (decimal?)c.GrandTotal,
                         check = a.CheckNo,
                         bank = a.Bank,
                         remark = a.Note,
            ...
            v = v.OrderByDescending(c => c.Date).Take(10);
            var data = v.ToList();
            return Json(new { data = data });
        }
        ...
        public JsonResult GetReminder()
        {
            ...
            Int32? newday = -5;
            newdate = today.Add(TimeSpan.FromDays((Int32)day));
            olddate = today.Add(TimeSpan.FromDays((Int32)newday));
            ...
            v = v.OrderByDescending(c => c.RDate).Take(10);
            var data = v.ToList();
            return Json(new { data = data });
        }
        ...
        public ActionResult Menu(string id = "")
        {
            ...
            var EnableUserMC = db.EnableSettings.Where(a => a.EnableType == "MenuColor").FirstOrDefault();

            if (EnableUserMC == null || EnableUserMC.TypeValue == null)
            {
                string MenuColors = "#0d83dd";
                addMethod("MenuColor", Status.active, MenuColors);
            }
            else
            {
                ViewBag.MenuColor = EnableUserMC.TypeValue;
            }

            var EnableHoverrMC = db.EnableSettings.Where(a => a.EnableType == "MenuOverColor").FirstOrDefault();
            if (EnableHoverrMC == null || EnableHoverrMC.TypeValue == null)
            {
                string MenuHoverColors = "#6c7cc6";
                addMethod("MenuOverColor", Status.active, MenuHoverColors);
            }
            else
            {
                ViewBag.MenuOverColor = EnableHoverrMC.TypeValue;
            }
            ...
            if (BusinessType == "Property")
            {
                ...
                return PartialView("_PropertyMenu", vmodel);
            }
            else
            {
                var user = User.Identity.GetUserId();
        ...
        public ContentResult displayuser()
        {
