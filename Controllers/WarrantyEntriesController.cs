...
{
    [RedirectingAction]
    public class WarrantyEntriesController : BaseController
    {
        // GET: Warranty
        ApplicationDbContext db;
        Common com;
        public WarrantyEntriesController()
        {
        ...
        public ActionResult downloadprint(long quotationId)
        {
            ...
            WarrantyEntries Quoentry = db.WarrantyEntries.Find(quotationId);
            ...
            string qedate = Quoentry.WDate.ToString("dd-MM-yyyy");
                ...
                var QuotData = com.WarrantyEntryData(quotationId, InPrintItemCode, PartNoCheck, 1000, ProjectCheck, ComHeadCheck);
           ...
           bool stat = true;
               // return new QuickSoft.Models.LegacyJsonResult { Data = new { status = stat, item, summary, billsundry, fmapp, qtnid = quotationId } };
        ...
        public ActionResult AddRemark(long? id)
        {
            ...
            WarrantyEntries cus = db.WarrantyEntries.Find(id);

            if (cus == null)
            {
                return NotFound();
            }
            var AmcRemarks = new AddedRemarks
            {
                TransactionId = cus.WarrantyId,
                TransactionType = "Warrentyremarks"
        ...
        [HttpPost]
        public JsonResult AddRemark(AddedRemarks amcremarks)
        {
            ...
            if (ModelState.IsValid)
            {
                if (amcremarks.Remarks != null)
                {
                    ...
                    WarrantyEntries AmcObj = db.WarrantyEntries.Find(amcid);
                    ...
                    com.addlog(LogTypes.Created, UserId, "warrantyremarks", "warranty", findip(), amcid, "AMC Remarks Added Successfully..");
        ...
        public ActionResult GetAllRemarksadded(long? RequisitionId)
        {
            ...
            var v = (from a in db.AddedRemarks
                     ...
                     where a.TransactionId == RequisitionId && a.TransactionType == "Warrentyremarks" && a.Remarks != null && a.CreatedDate >= rmdate
        ...
        public ActionResult Delete(long? id)
        {
            ...
            WarrantyEntries Obj = db.WarrantyEntries.Find(id);
        ...
        public ActionResult DeleteAction(long id)
        {
            ...
            {
                stat = true;
                msg = "Successfully deleted Warrentry.";
            }
            else
            {
                stat = false;
        ...
        private Boolean DeleteEntry(long AmcId)
        {
            ...
            var AmcRemarks = db.AmcRemarks.Where(a => a.TransId == AmcId && a.TransType == "Warrentyremarks");
            if (AmcRemarks != null)
            {
                db.AmcRemarks.RemoveRange(db.AmcRemarks.Where(a => a.TransId == AmcId && a.TransType == "Warrentyremarks"));
                db.SaveChanges();
            }

            //***********Delete from table Amc
            WarrantyEntries MastObj = db.WarrantyEntries.Find(AmcId);
            db.WarrantyEntries.Remove(MastObj);
            db.SaveChanges();
        ...
        public ActionResult Details(long id)
        {
            ...
            vmodel = (from a in db.WarrantyEntries
                      
                      join d in db.Customers on a.Customer equals d.CustomerID
                      
                      where a.WarrantyId == id
                      select new
                      {
                          a.WarrantyId,
                          a.BillNo,
                      ...
                      }).ToList().Select(o => new QuotationViewModel
                      {
                          QuotationId = o.WarrantyId,
                          BillNo = o.BillNo,
                         QuotDate=o.WDate,
            ...
            vmodel.QuotItem = db.WEItems.Where(a => a.Warranty == id).Select(b => new QuotItemViewModel
            {
        ...
        public ActionResult GetWarranty(long? openclose,string BillNo, string FromDate, string ToDate, long? customer, long? salesperson)
        {
            var draw = Request.Form.GetValues("draw").FirstOrDefault();
            int recordsTotal = 0;
            ...
            var UserId = User.Identity.GetUserId();
            bool chkop = (openclose == 1) ? true : false;
            // dc.ChangeTracker.LazyLoadingEnabled = false; // if your table is relational, contain foreign key
            var v = (from b in db.WarrantyEntries
                     ...
                     (customer == 0 || customer == null || b.Customer == customer) &&
                      (openclose == null || b.openclose == chkop) &&
                     ...
                     select new
                     {
                         ...
                         b.WTaxAmount,
                         b.WItemQuantity,
                         Salesman=e.FirstName+" "+e.MiddleName+" "+e.LastName,
        ...
        public ActionResult Edit(long? id)
        {
            ...
            ViewBag.image = (from b in db.quotationdocuments
                             join c in db.WarrantyEntries on b.quotationID equals c.WarrantyId
                             where c.WarrantyId == id
      ...
      CustomerDetails = s.CustomerCode + " - " + s.CustomerName
        ...
        public JsonResult Edit(string[][] array, string[] quotdata, string action, ICollection<QtBillSundry> bsmodel)
        {
            ...
            if (quotdata[16] != null)
            {
                ...
                WarrantyEntries Quoentry = db.WarrantyEntries.Find(quotEntryId);
                ...
                if (ModelState.IsValid)
                {
                    ...
                    Quoentry.openclose = quotdata[40] == "1" ? true : false;
                    Quoentry.logtime = System.DateTime.Now;
                    db.Entry(Quoentry).State = EntityState.Modified;
                    db.SaveChanges();

                    ////// add to SEItem
                    var itm = db.WEItems.Where(a => a.Warranty == quotEntryId).FirstOrDefault();
                    if (itm != null)
                    {
                        db.WEItems.RemoveRange(db.WEItems.Where(a => a.Warranty == quotEntryId));
                        db.SaveChanges();
                    }
                    foreach (var arr in array)
                    {
                        WEItems witem = new WEItems();
                        witem.ItemUnit = Convert.ToInt64(arr[1]);
                        witem.ItemUnitPrice = (arr[3] == "") ? 0 : Convert.ToDecimal(arr[3]);
                        witem.ItemQuantity = (arr[2] == "") ? 0 : Convert.ToDecimal(arr[2]);
                        witem.ItemSubTotal = Convert.ToDecimal(arr[5]);
                        witem.ItemDiscount = Convert.ToDecimal(arr[6]);
                        witem.ItemTax = Convert.ToDecimal(arr[10]);
                        witem.ItemTaxAmount = Convert.ToDecimal(arr[9]);
                        witem.ItemTotalAmount = Convert.ToDecimal(arr[11]);
                        witem.WarrantyPeriod = arr[13];
                        if (arr.Length > 30)
                            witem.itemNote = Convert.ToString(arr[33].Replace("\n", "<br />"));
                        else
                            witem.itemNote = "";
                        witem.Warranty = quotEntryId;
                        witem.Item = Convert.ToInt32(arr[0]);

                        db.WEItems.Add(witem);
                        db.SaveChanges();
                    ...
                    if (action == "print")
                    {
                        ...
                        var QuotData = com.WarrantyEntryData(quotEntryId, InPrintItemCode, PartNoCheck, TimeOut, ProjectCheck, ComHeadCheck);
                        var item = QuotData.pdfItem.ToList();
                        var summary = QuotData;
                        var billsundry = QuotData.billsundry.ToList();
        ...
        {
            var Exists = db.WarrantyEntries.Any(c => c.BillNo == QENo);
   ...
   new List<SelectListItem>
   {
        ...
        public JsonResult Create(string[][] array, string[] quotdata, string action, ICollection<QtBillSundry> bsmodel)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    stat = false;
                    msg = "Same Warrenty Number Already Exists. please increase warrenty No";
                    return new QuickSoft.Models.LegacyJsonResult { Data = new { status = stat, message = msg } };
                }
                ...
                //sales entry
                WarrantyEntries Quoentry = new WarrantyEntries();
                if (quotdata[19] != null)
                {
                ...
                Quoentry.WGrandTotal = Convert.ToDecimal(quotdata[7]);
                Quoentry.WNote = Convert.ToString(quotdata[11]);
                Quoentry.openclose = quotdata[42] == "1" ? true : false;
                Quoentry.logtime = System.DateTime.Now;
                ...
                db.WarrantyEntries.Add(Quoentry);
                db.SaveChanges();
                Int64 quotationId = 0;
                Int64 checkid = 0;
                string check = "true";
                ...
                if (action == "print")
                {
                    ...
                    var QuotData = com.WarrantyEntryData(quotationId, InPrintItemCode, PartNoCheck, TimeOut, ProjectCheck, ComHeadCheck);
                    var item = QuotData.pdfItem.ToList();
                    var summary = QuotData;
                    var billsundry = QuotData.billsundry.ToList();
                ...
                else if (action == "sendmail")
                {
                    ...
                    msg = "Successfully submitted WarrentyEntries.";
                    stat = true;
        ...
        private string InvoiceNo2(Int64 PENo = 0, string billNo = null, string section = null)
        {
            ...
            if (billNo == null)
            {
                ...
                var num = db.WarrantyEntries.OrderByDescending(p => p.WarrantyId).Select(p => p.BillNo).FirstOrDefault();
                if (num == null)
                {
