    ...
    public class PurchaseReturnController : BaseController
    {
        ...
        public JsonResult CreatePurchaseReturn(string[][] array, string[] purchasedata, PRBillSundryViewModel bsmodel, ICollection<BatchStockPViewModel> bstmodel)
        {
            ...
            if (!BillExist(Convert.ToString(purchasedata[16])))
            {
                ...
                PRentry.Ref5 = Convert.ToString(purchasedata[27]);
                ...
                //// execute sp sql 
                string sql = String.Format("EXEC {0} {1};", "SP_InsertPRItems", "@TableType");
                //// execute sql 
        ...
        public ActionResult GetPurchaseReturn(string BillNo, string FromDate, string ToDate, long? supplier, long? salesperson, long? type, string user, int? Balance, long? MC, string appstat, string PurchaseType)
        {
            ...
            var tem = 0;
            var userEditDays = db.UserEditDayss.Where(a => a.userid == UserId).Select(a => a.prdays).FirstOrDefault();
            ...
            var serverQuery = (from a in db.PurchaseReturns
             ...
             //        && ((MCArray.Contains(a.MaterialCenter) && MC == a.MaterialCenter) || ((MC == null) && MCArray.Contains(a.MaterialCenter))) //&& (!MCList.Any() || MCArray.Contains(a.MaterialCenter))
                  //   && (userpermission == true || a.CreatedBy == UserId)
                     ...
                     select new
                     {
                         validornot = tem != 1 && (EF.Functions.DateDiffMinute(a.PRCreatedDate, editableDay) <= 0 && EF.Functions.DateDiffMinute(a.PRCreatedDate, today) >= 0) ? "valid" : "invalid",
                         a.PurchaseReturnId,
                         ...
                         PReturnAmount = c != null ? c.PReturnAmount : 0,
                         a.Remarks,
                         BalanceAmt = c != null ? a.PRGrandTotal - c.PReturnAmount : 0,
                         Dev = uDev,
        ...
        public ActionResult Details(long? id)
        {
            ...
            vmodel.PRItemss = db.PRItemss.Where(a => a.PurchaseReturnId == id && a.itemNote != "-:{Bundle_Item}")
            .Select(b => new PRItemViewModel
            {
                ...
                bundleitem = (from ab in db.PRItemss
        ...
        public ActionResult Edit(long? id)
        {
            ...
            ViewBag.Image = (from a in db.AttachmentDocuments
                             join b in db.PurchaseReturns
                             on a.TransactionID equals b.PurchaseReturnId
                             where a.TransactionID == id && a.TransactionType == "Purchase Return"
                             select new PurchaseRtnDocumentViewModel
                             {
                                 DocumentID = a.DocumentID,
                                 PurchRtnId = a.TransactionID,
                                 FileName = a.FileName,
            ...
            var DItem = db.DummyPRItems.Where(a => a.PurchaseReturnId == id).FirstOrDefault();
            var PItem = db.PRItemss.Where(a => a.PurchaseReturnId == id).FirstOrDefault();
            if (PItem == null && DItem != null)
            {
                var DItems = db.DummyPRItems.Where(a => a.PurchaseReturnId == id).ToList();
                foreach (var arr in DItems)
                {
                    //add to se-item table
                    PRItems sItem = new PRItems();
                    ...
                    sItem.PurchaseReturnId = arr.PurchaseReturnId;
                    sItem.Item = arr.Item;
                    db.PRItemss.Add(sItem);
                    db.SaveChanges();
                }

                db.DummyPRItems.RemoveRange(db.DummyPRItems.Where(a => a.PurchaseReturnId == id));
                db.SaveChanges();
            }
        ...
        {
            var ConD = (from a in db.PRItemss
                        ...
                        select new
                        {
                            ...
                            batch = (from ay in db.BatchStocks
                                     join az in db.PRItemss on new { f1 = ay.Item, f2 = ay.Unit, f3 = ay.Reference, f4 = ay.Type }
        ...
        public JsonResult UpdatePurchaseReturn(string[][] array, string[] purchasedata, PRBillSundryViewModel bsmodel, ICollection<BatchStockPViewModel> bstmodel)
        {
            ...
            if (com.chkApproved(purchaseReturnId, EditPermission, "PurchaseReturn", UserId) == true)
            {
                ...
                PRentry.Ref5 = Convert.ToString(purchasedata[28]);
                PRentry.PReturnAccount = purRAcc;
                ...
                var PRItem = db.PRItemss.Where(a => a.PurchaseReturnId == purchaseReturnId).FirstOrDefault();
                if (PRItem != null)
                {
                    var PItems = db.PRItemss.Where(a => a.PurchaseReturnId == purchaseReturnId).ToList();
                    ...
                    db.PRItemss.RemoveRange(db.PRItemss.Where(a => a.PurchaseReturnId == purchaseReturnId));
                    db.SaveChanges();
                }
                ...
                if (ret > 0)
                {
                    db.DummyPRItems.RemoveRange(db.DummyPRItems.Where(a => a.PurchaseReturnId == purchaseReturnId));
                    db.SaveChanges();
                }

                // batch stock

                var SEBst = db.BatchStocks.Where(a => a.Reference == purchaseReturnId && a.Type == "PReturn").FirstOrDefault();
                if (SEBst != null)
                {
                    db.BatchStocks.RemoveRange(db.BatchStocks.Where(a => a.Reference == purchaseReturnId && a.Type == "PReturn"));
                    db.SaveChanges();
                }
                if (bstmodel != null)
                {
        ...
        public StringBuilder generatePdf(long PurchaseReturnId)
        {
            ...
            return com.generatepdf(PurchaseReturnId, summary, item, billsundry, "PurchaseReturn");
        }
        ...
        private Boolean DeletePReturn(long id)
        {
            ...
            var PRItem = db.PRItemss.Where(a => a.PurchaseReturnId == id);
            ...
            if (PRItem != null)
            {
                db.PRItemss.RemoveRange(db.PRItemss.Where(a => a.PurchaseReturnId == id));
        ...
        private string InvoiceNo(Int64 PRNo = 0, string billNo = null)
        {
            ...
            if (billNo == null)
            {
                ...
                {
                    PRNo = db.PurchaseReturns.Max(p => p.PRNo + 1);
        ...
        public ActionResult UploadFiles()
        {
            ...
            {
                string PurchRtnId = Request.Form.GetValues("id").First();
                long PRId = 0;

                if (PurchRtnId.Contains("undefined"))
                {
                    var LastId = db.PurchaseReturns.OrderByDescending(a => a.PurchaseReturnId).FirstOrDefault();
                    PRId = LastId.PurchaseReturnId;
                }
                else
                {
                    PRId = Convert.ToInt64(PurchRtnId);
                }
                ...
                try
                {
                    ...
                    if (files.Count > 0)
                    {
                        string path = LegacyWeb.MapPath("~/uploads/PurchaseReturnDocuments/");
                        ...
                        for (int i = 0; i < files.Count; i++)
                        {
                            ...
                            if (file.Length > 0)
                            {
                                ...
                                if (extension == ".jpg" || extension == ".jfif" || extension == ".png" || extension == ".jpeg")
                                {
                                    ...
                                    thumbName = Path.Combine(LegacyWeb.MapPath("~/uploads/PurchaseReturnDocuments/"), thumbName);

                                    resizeName = "resize_" + newName.Split('.').ElementAt(0) + "." + newName.Split('.').ElementAt(1);
                                    resizeName = Path.Combine(LegacyWeb.MapPath("~/uploads/PurchaseReturnDocuments/"), resizeName);
                                ...
                                newName = Path.Combine(LegacyWeb.MapPath("~/uploads/PurchaseReturnDocuments/"), newName);
                                file.SaveAs(newName);
        ...
        public JsonResult ImageDelete(long key)
        {
            ...
            string fullpath = LegacyWeb.MapPath("~/uploads/PurchaseReturnDocuments/" + Document.FileName);
            ...
            var UserId = User.Identity.GetUserId();
            com.addlog(LogTypes.Deleted, UserId, "PurchaseReturn", "AttachmentDocuments", findip(), Document.DocumentID, "Purchase Return Document Deleted Successfully");

            bool status = true;
            string message = "Successfully deleted Purchase Return attachment details.";
