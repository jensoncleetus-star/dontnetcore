...
var flagbil = 0;
var mainflag = 0;
function addrow(t, action,Warranty, ItemUnit, ItemTax, ItemTotalAmount, ItemQuantity, Item, ItemCode, ItemName, ItemUnitPrice, ItemSubTotal, ItemWithCode, ItemTaxAmount, ItemDiscount, ItemNote, itemdata, ConFactor, ItemCategory, ItemBrand, PPRICE, MRP, SellPrice, BPrice, tag1, tag2, tag3, tag4, tag5, prefix, division, supplier, PricingStrategy) {
    ...
    if (count == limits) alert("You have reached the limit of adding " + count + " inputs");
        ...
        var searchbtn = "<span class='input-group-btn'><a type='button' href='/Item/Searchmodal' class='modal-create-lg btn btn-success btn-flat'><i class='fa fa-search' aria-hidden='true'></i></a></span>"
        searchbtn2 = "<a type='button' href='/WorkCompletion/AddMultiInvoice' class='modal-create-lg btn btn-success btn-flat'><i class='fa fa-1x fa-search-plus'></i></a>"
        //var itemaddbtn = "<span class='input-group-btn'><a type='button' href='/Item/AddItem' class='modal-create-lg btn btn-success btn-flat'><i class='fa fa-1x fa-plus-circle'></i></a>" + notbtn + "</span>";
        ...
        data = "<td class='text-center' id=" + divid + "> " + slno + " </td>" +
            ...
            "<td style='width:50px;'> <input type='number' name='product_quantity[]' data-msg-min ='The Item Quantity must be Greater than Zero' onblur='quantity_change(" + count + ");' id='total_qntt_" + count + "' value='" + ItemQuantity + "'  class='total_qntt_" + count + " form-control text-right quty' placeholder='0' value='0' min='.01' tabindex='" + tab2 + "'/></td>" +
            TInRate +
            ...
            "<td> <input type='text' id='total_warntyprd_" + count + "' value='" + Warranty + "'  class='total_warntyprd_" + count + " form-control text-right' placeholder='0' value='0' tabindex='" + tab2 + "'/></td>" +
...
function bindItem(e, dvitem, type) {
    ...
    function ItemsBind(ritem, rtype, bcount) {
        ...
        if (dvitem != "active" && rtype != "bundle") {
            dvField1 += '<td class="text-right" style="display:none;">' + bold + parseFloat(ritem.ItemUnitPrice).toFixed(2) + boldend + '</td>';
            ...
            dvField2 += '<td class="text-right" style="display:none;">' + bold + parseFloat(ritem.ItemUnitPrice * ritem.ItemQuantity).toFixed(2) + boldend + '</td>';
        ...
        if (Layout == "Jewellery") {
            ...
            Row += '<td>' + ritem.ItemQuantity + '</td>';
            Row += dvField1 + dvField2;
            Row+='<td>' + ritem.WarrantyPeriod + ' Year </td>';
        }
        Row += '</tr>';
        return Row;
    }
...
function PrintInvoice(e, type, dvitem, conType) {
    ...
    $("#lblRef1").text(e.summary.Ref1);
    $("#lblRef2").text(e.summary.Ref2);
    ...
    if (Layout == "Scaffold") {
        ...
        if ((conType == "DeliveryNote" || conType == "ProForma" || conType == "SalesReturn" || conType == "SalesEntry" || conType == "Quotation" || conType == "SalesOrder" || conType == "WorkCompletion"|| conType=="Warranty")) {
        ...
        $("#customername").text(e.summary.PartyName);
        $("#wdate").text(convertToDate(e.summary.Date));
    ...
    switch (conType) {
        ...
        case 'SalesOrder':
            ...
            break;
        case 'WorkCompletion':
            var titname = "Work Completion - " + e.summary.PartyName + " - " + e.summary.BillNo;
            $('title').html(titname);
            break;
        case 'Warranty':
            var titname = "Warranty - " + e.summary.PartyName + " - " + e.summary.BillNo;
            $('title').html(titname);
            break;
        default:
            var titname = "Tax Invoice - " + e.summary.PartyName + " - " + e.summary.BillNo;
    ...
    //window.MyHandler.createWebPagePrint();
