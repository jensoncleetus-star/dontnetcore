...
function addRackPURRow(arg, ItemId, BtData, mcid=1) {
    if (rackcount == sblimits) alert("You have reached the limit of adding " + rackcount + " inputs In Batch Stock");
        ...
        if (BtData) {
            ...
            RackStockIn = (sbunit != punit) ? BtData.StockIn : acSt;
        ...
        var rackOption = "<option value='" + RackNo + "'>" + RackNo + "</option>";
        var shelfOption = "<option value='" + shelfno + "'>" + shelfno + "</option>";

        var sbtcost = gtTot;
        var row = "<tr class='Bst_" + slno + "'>";
        data = "<td class='text-center'>" + slno + "</td>" +
            "<td><select data-name='RackNo' data-item='" + ItemId + "' data-count='" + rackcount + "' class='bts_rackno_" + rackcount + " bts_rackno form-control' placeholder='rackno' onchange='btsRack_change(this," + arg + ",\"" + ItemId + "\");' required='required'>" + rackOption + "</select> </td>" +
            ...
            "<td><input type='number' data-name='StockOut' data-count='" + rackcount + "' data-msg-min ='The Item Quantity must be Greater than Zero' onchange='btsqty_change(this," + arg + ",\"" + ItemId + "\");' class='bts_qty_" + rackcount + " bts_qntt form-control text-right' placeholder='0' value='" + RackStockIn + "' min='0' data-max='' required='required' /></td>" +
            "<td class='text-center'><button data-count='" + rackcount + "' class='btn btn-danger' type='button' value='Delete'  onclick='deleteBtsRow(this,\"" + arg + "\")'><i class='fa fa-trash fa-1x'></i></button>" +
