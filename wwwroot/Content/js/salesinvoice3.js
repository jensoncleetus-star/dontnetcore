...
function addrow(t, action, ItemUnit, ItemTax, ItemTotalAmount, ItemQuantity, Item, ItemCode, ItemName, ItemUnitPrice, ItemSubTotal, ItemWithCode, ItemTaxAmount, ItemDiscount, ItemNote, itemdata, ConFactor, ItemCategory, ItemBrand, PPRICE, MRP, SellPrice, BPrice, tag1, tag2, tag3, tag4, tag5, prefix, division, supplier) {
    ...
    if (count == limits) alert("You have reached the limit of adding " + count + " inputs");
        ...
        var itemaddbtn = "<span class='input-group-btn'><a type='button' href='/Item/AddItem' id='sh-btn-add' class='modal-create-lg btn btn-success btn-flat'><i class='fa fa-1x fa-plus-circle sh-icon1'></i></a>" + notbtn + "</span>";
        ...
        data = "<td class='text-center' id=" + divid + "> " + slno + " </td>" +
            ...
            "<td style='width:50px;'>Qty <input type='number' name='product_quantity[]' data-msg-min ='The Item Quantity must be Greater than Zero' onchange='quantity_change(" + count + ");' id='total_qntt_" + count + "' value='" + ItemQuantity + "'  class='total_qntt_" + count + " form-control text-right quty' placeholder='0' value='0' min='.01' tabindex='" + tab2 + "'/></td>" +
            TInRate +
            "<td>Rate<input type='number' name='product_rate[]' " + required + RateReadOnly + " data-msg-required='The Item Rate field is required' onchange='rate_change(" + count + ",\"" + type + "\");' id='price_item_" + count + "' value='" + ItemUnitPrice + "' class='price_item_" + count + " form-control text-right totrate' placeholder='0.00' min='0' tabindex='" + tab3 + "'/><input type='hidden' data-value='" + price + "' value='" + baseprice + "' name='base_rate' id='base_rate_" + count + "'> </td> " +
            "<td>Sub.Total<input type='number' name='sub_total[]' id='sub_total_" + count + "' class='sub_total_" + count + " form-control text-right subtotal' value='" + ItemSubTotal + "'   placeholder='0.00' min='0' tabindex='" + tab3 + "' readonly='readonly'/></td>" +
            "<td>Discount<input type='number' name='item_discount[]' id='item_discount" + count + "' onchange='itemdiscount_change(" + count + ",\"" + type + "\");' class='item_discount" + count + " form-control text-right item_discount' value='" + ItemDiscount + "' placeholder='0.00' tabindex='" + tab3 + "'/></td>" +
            "<td class='vwise'>Tax<input type='text' id='tax_" + count + "' class='form-control text-right tax tax_" + count + "' tabindex='" + tab4 + "' readonly='readonly' /><input type='hidden' class='item_amount' name='item_amount' id='item_amount_" + count + "'/><input type='hidden' class='tot_tax' name='tot_tax' id='tot_tax_" + count + "'/><input type='hidden'  class='tax_percentage' value='" + ItemTax + "' name='tax_percentage' id='tax_percentage_" + count + "'/></td> " +
            "<td  style='text-align:center' class='text-right'>G.Total<input style='text-align:center!important;' class='total_price total_price_" + count + " form-control text-right' type='text' name='total_price[]' value='" + ItemTotalAmount + "' id='total_price_" + count + "' value='0.00' readonly='readonly'/><input type='hidden' value='" + ConFactor + "' class='cfactor' name='cfactor' id='cfactor_" + count + "'/> " +
...
function addLastSPRow(ItemId) {
    ...
    if (salecount > 0) {
        ...
        var oTable = $('#LSale-' + ItemId + '').DataTable({
            ...
            "columns": [
                {
                    "data": "SEItemsId", "name": "SEItemsId", "sClass": "headeralign",
    ...
    if (purchasecount > 0) {
        ...
        var oTable = $('#LPurchase-' + ItemId + '').DataTable({
            ...
            "columns": [
                {
                    "data": "PEItemsId", "name": "PEItemsId", "sClass": "headeralign",
                    "render": function (data, type, row, meta) {
                        return meta.row + meta.settings._iDisplayStart + 1;
                    }
                },
...
function rate_change(arg, type, foredit) {
    ...
    var purvalue = $("#selitem_pprice_" + arg).val();
    ...
    if (index == 1) {
        ...
        if ((parseFloat(rate) < parseFloat(purvalue)) && parseInt(rate) > 0) {
...
function bindItem(e, dvitem, type) {
    ...
    function ItemsBind(ritem, rtype, bcount) {
        ...
        if (dvitem != "active" && rtype != "bundle") {
            ...
            dvField2 += '<td class="text-right" style="display:none;">' + bold + parseFloat(ritem.ItemTaxAmount).toFixed(2) + boldend + '</td>';
            dvField2 += '<td class="text-right">' + bold + parseFloat(ritem.ItemUnitPrice * ritem.ItemQuantity).toFixed(2) + boldend + '</td>';
                ...
                if (Layout == "Jewellery" || Layout == "Scaffold") {
                    ...
                    if (rt != "" && rt != null) {
                       // itemdetail = "<span>" + "S/N : " + ritem.BatchNo + " <br />" + ritem.ItemName + "</span>" + itemnote;
                        itemdetail = "<span>" + "S/N : " + rt + " <br />" + ritem.ItemName + "</span>" + itemnote;
                    }
                    else {
...
function GenerateSettlementMode(fnval, SaveMode, EmpId) {
    ...
    //var ProTaskId       =   $('#ddlProTask').val();

    if (SaveMode == "Edit") {
        SalesEntryID    =   getQueryString('');
        TotalAmount     =   InvoiceAmount;        

       // $("#btnSettlementCancel").attr('disabled', 'disabled')
    }
    ...
    wrapper1Data = '<div class="row PMaint">' +
                        ...
                        //'<input type = "hidden" name="Addon[0].CustomerId"  value="' + CustId + '" />' +
                        //'<input type = "hidden" name="Addon[0].ProjectId"   value="' + ProjectId + '" />' +
                        //'<input type = "hidden" name="Addon[0].ProTaskId"   value="' + ProTaskId + '" />' +
                   '</div >'
    ...
    $.ajax({
        ...
        dataType: 'json',
        type: "POST",
        data: { EmployeeId: EmpId, CustomerId: CustId, SalesEntryId: SalesEntryID, Mode: SaveMode },
        cache: true,
        success: function (data) {
                        ...
                        '<div class="row PymntDetails" style="margin-top: 1rem;">' +
                            ...
                            '<div class="form-group col-md-1" style="margin-right:-20px; margin-left:20px;">' +
                                ...
                                '<div style="margin-top:10px;">' +
                                    '<input type="checkbox"  id="CheckBox' + i + '"  onchange="ChangeReadOnly(this, ' + i + ');"  > ' +
                                '</div>' +
                            '</div>' +
...
//Function To Save The Settlement Details
//function SaveSettlementDetails(SalesEntryId) {

//    var url     =   $('#UpdateSettlement')[0].action;
//    var data    =   $('#UpdateSettlement').serialize();

//    $.ajax({
//        type: "POST",
//        url: url,
//        data: data + '&SalesEntryId=' + SalesEntryId,
//        success: function (data) {
//            if (data.status) {
