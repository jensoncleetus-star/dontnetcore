...
namespace QuickSoft.Areas.Property.Controllers
{
    ...
    public class TenantController : BaseController
    {
        ...
        [HttpPost]
        //[QkAuthorize(Roles = "Dev,Tenant")]
        public JsonResult GetTenant(long? Tenant, string TaxReg, long? Mobile, long? Phone, decimal? CLimit, int? CPeriod, long? Employee, string TxType, string MailId, string Alias)
        {
            ...
            var userpermission = User.IsInRole("All Tenant");
            var UserId = User.Identity.GetUserId();

            var uDev = User.IsInRole("Dev");
            var uCustView = User.IsInRole("View Tenant");
            var uEdit = User.IsInRole("Edit Tenant");
            var uDelete = User.IsInRole("Delete Tenant");

            var v = (from a in db.Tenants
                     ...
                     where
                           (Tenant == null || Tenant == 0 || a.TenantID == Tenant) &&
            ...
            {
                // Apply search   
                v = v.Where(p => p.TenantName.ToString().ToLower().Contains(search.ToLower()) ||
                                 p.TenantCode.ToString().ToLower().Contains(search.ToLower()) ||
            ...
            // EF Core 10 cannot translate the nested mobmodel collection-projection once Tenants have rows.
            ...
            var data = page.Select(r => new {
                r.id, r.TenantCode, r.TenantName, r.TaxRegNo, r.Location, r.Address, r.Phone, r.Email,
        ...
        public ActionResult Create()
        {
            ...
            ViewBag.LastEntry = db.Tenants.Select(p => p.TenantID).AsEnumerable().DefaultIfEmpty(0).Max();
            TenantViewModel vmodel = new TenantViewModel();
            vmodel.TenantCode = CustCode();
        ...
        public JsonResult Create(TenantViewModel vmodel)
        {
            string msg = "";
            bool stat = false;
            var custExists = db.Tenants.Any(u => u.TenantCode == vmodel.TenantCode);
            if (custExists)
            {
                msg = "Tenant Name already exists.";
                stat = false;
                //Danger("A Tenant with same Tenant Code exists.", true);
            ...
            else
            {
                ...
                if (ModelState.IsValid)
                {
                    var bro = db.companys.Select(a => a.Tenant).FirstOrDefault();
                    if (bro == null)
                    {
                        msg = "Please give Account Group for Tenant in Configuration.";
                        stat = false;
                    ...
                    account.Name = vmodel.TenantName;
                    account.Alias = vmodel.Alias;
                    account.PrintName = vmodel.TenantName;
                    ...
                    if (vmodel.TenantID != 0 && vmodel.TenantID != null)
                    {
                        Tenant cus = db.Tenants.Find(vmodel.TenantID);
                        cus.Accounts = account.AccountsID;
                        cus.TenantName = vmodel.TenantName;
                        cus.TenantCode = vmodel.TenantCode;
                    ...
                    else
                    {
                        ...
                        Tenant cus = new Tenant
                        {
                            EntryNo = GetEntryNo(),
                            Contact = contactId,
                            Accounts = accountId,
                            TenantName = vmodel.TenantName,
                            TenantCode = vmodel.TenantCode,
                        ...
                        foreach (var arr in vmodel.docmodel)
                        {
                            if (1==1)
                            {
                                ...
                                if (arr.Attachments != null)
                                {
                                    if (arr.Attachments != null)
                                    {
                                        string storePath = LegacyWeb.MapPath("~/uploads/TenancyContractDocument/" + "Tenant_" + docid);
                                        ...
                                        var uploadUrl = LegacyWeb.MapPath("~/uploads/TenancyContractDocument/" + "Tenant_" + docid + "/");
                    ...
                    com.addlog(LogTypes.Created, UserId, "Tenant", "Tenant", findip(), CustId, "Tenant Added Successfully");

                    msg = "Successfully added Tenant details.";
                    stat = true;
                    return new QuickSoft.Models.LegacyJsonResult { Data = new { status = stat, message = msg } };
                }
                else
        ...
        public Boolean InsertToSales(ReferenceAccountViewModel arr, long CustId, string UserId, long branch)
        {
            ...
            SEentry.Remarks = "Sales Entry From Tenant Creation";
        ...
        public ActionResult Edit(long? id)
        {
            ...
            var cus = (from a in db.Tenants
                       join b in db.Accountss on a.Accounts equals b.AccountsID
                       where a.TenantID == id && (userpermission == true || b.CreatedBy == b.CreatedBy)
                       select new
                       {
                           a.Contact,
                           a.Accounts,
            ...
            TenantSubmitViewModel landmodel = new TenantSubmitViewModel();
            landmodel.TenantID = cus.TenantID;
            landmodel.TenantName = cus.TenantName;
            landmodel.TenantCode = cus.TenantCode;
            ...
            var cust = db.Tenants.Select(r => new
            {
                ID = r.TenantName,
                Name = r.TenantName
            }).ToList();
            ...
            ViewBag.preEntry = db.Tenants.Where(a => a.TenantID < id && (userpermission == true)).Select(a => a.TenantID).DefaultIfEmpty().Max();
            ViewBag.nxtEntry = db.Tenants.Where(a => a.TenantID > id && (userpermission == true)).Select(a => a.TenantID).DefaultIfEmpty().Min();
        ...
        [HttpPost]
        //[QkAuthorize(Roles = "Dev, Create Tenant")]
        public JsonResult Update(TenantSubmitViewModel cusmodel)
        {
            bool stat = false;
            string msg;
            var UserId = User.Identity.GetUserId();
            var Branch = db.Users.Where(a => a.Id == UserId).Select(a => a.BranchID).FirstOrDefault();
            var id = cusmodel.TenantID;
            if (!ModelState.IsValid)
            {
                var modelErrors = new List<string>();
                foreach (var modelState in ModelState.Values)
            ...
            if (ModelState.IsValid)
            {
                var CodeExists = db.Tenants.Any(u => u.TenantCode == cusmodel.TenantCode && u.TenantID != id);
                if (CodeExists)
                {
                    msg = "A Tenant with same Tenant Code exists.";
                    stat = false;
                }
                else
                {
                    Tenant cus = db.Tenants.Find(id);
                    cus.TenantName = cusmodel.TenantName;
                    cus.TenantCode = cusmodel.TenantCode;
                    ...
                    db.Entry(cus).State = EntityState.Modified;
                    db.SaveChanges();
                    Int64 CustId = cus.TenantID;

                    Contact cont = db.Contacts.Find(cus.Contact);

                    cont.Name = cusmodel.TenantName;
                    ...
                    Accounts account = db.Accountss.Find(cus.Accounts);
                    account.PrintName = cusmodel.TenantName;
                    account.Name = cusmodel.TenantName;
                    ...
                    var bro = db.companys.Select(a => a.Tenant).FirstOrDefault();//db.EnableSettings.Where(a => a.EnableType == "Tenant").Select(a => a.TypeValue).FirstOrDefault();
                    account.Group = (bro == null) ? 0 : Convert.ToInt64(bro);
                    ...
                    //              where a.TenantId == id && a.Recieptid == 0
                    ...
                    //    //delete other sales
                    //    var salelist = db.SalesEntrys.Where(a => a.Tenant == CustId && a.Status == 0).ToList();
                    ...
                    foreach (var arr in cusmodel.docmodel)
                    {
                        ...
                        if (Convert.ToInt32(arr.ID) == 0 && arr.Type != null)
                        {
                            ...
                            doc.DocumentType = Convert.ToInt64(arr.Type);
                            doc.Purpose = "Tenant";
                            doc.Reference = cusmodel.TenantID;
                            if (arr.Date != null)
                                doc.ExpDate = DateTime.Parse(arr.Date, new CultureInfo("en-GB"));
                            else
                                doc.ExpDate = null;
                    ...
                    com.addlog(LogTypes.Updated, UserId, "Tenant", "Tenants", findip(), cus.TenantID, "Tenant Updated Successfully");

                    msg = "Successfully updated Tenant details.";
                    stat = true;
                }
            }
            else
        ...
        public ActionResult DeleteAllTenant(long[] bill)
        {
            Int32 count = 0;
            Int32 notdel = 0;
            ...
            if (count > 0 && notdel > 0)
            {
                Success("Deleted " + count + " Tenants, Unable to Delete " + notdel + " Tenants. ", true);
            }
            else if (notdel > 0)
            {
                Danger("Unable to Delete " + notdel + " Tenants.", true);
            }
            else
            {
                Success("Deleted " + count + " Tenants.", true);
            }
        ...
        [RedirectingAction]
        //[Authorize(Roles = "Dev,Delete Tenant")]
        public ActionResult Delete(long? id)
        {
            ...
            Tenant ptype = db.Tenants.Find(id);
            if (ptype == null)
            {
                return NotFound();
            }
        ...
        public ActionResult DeleteConfirmed(long id)
        {
            ...
            {
                stat = DeleteFn(id);
                msg = "Successfully Deleted Tenant details.";
            }
            return new QuickSoft.Models.LegacyJsonResult { Data = new { status = stat, message = msg } };
        }
        ...
        {
            var UserId = User.Identity.GetUserId();
            db.PropertyDocumentTypes.RemoveRange(db.PropertyDocumentTypes.Where(a => a.Reference == id && a.Purpose == "Tenant"));
            db.SaveChanges();
            Tenant pt = db.Tenants.Find(id);

            db.Tenants.Remove(pt);
            db.SaveChanges();

            com.addlog(LogTypes.Deleted, UserId, "Tenant", "Tenant", findip(), pt.TenantID, "Tenant Deleted Successfully");
            return true;
        }
        ...
        [HttpGet]
        //[QkAuthorize(Roles = "Dev,View Tenant")]
        public ActionResult Details(int? id)
        {
            ...
            TenantSubmitViewModel cusmodel = new TenantSubmitViewModel();
            cusmodel = (from a in db.Tenants
                        join b in db.Contacts on a.Contact equals b.ContactID into tmp
                        from b in tmp.DefaultIfEmpty()
                        join e in db.Accountss on a.Accounts equals e.AccountsID
                        where a.TenantID == id
                        select new TenantSubmitViewModel
                        {
                            TenantID = a.TenantID,
        ...
        public JsonResult GetMobile(long CnId)
        {
            var ConD = (from a in db.Mobiles
                        join b in db.Tenants on a.Contact equals b.Contact
                        where b.TenantID == CnId
                        select new
                        {
                            Mob = a.MobileNum,
                            Name = a.Name
        ...
        public JsonResult SearchTenant(string q, string x)
        {
            List<SelectFormat> serialisedJson;
            if (!(string.IsNullOrEmpty(q) && string.IsNullOrEmpty(q)))
            {
                serialisedJson = (from a in db.Tenants
                                  where a.TenantName.ToLower().Contains(q.ToLower()) || a.TenantName.Contains(q)
            ...
            {
                serialisedJson = db.Tenants.Select(b => new SelectFormat
                {
                    text = b.TenantName,
                    id = b.TenantID
                }).OrderBy(b => b.text).ToList();
            ...
            {
                var initial = new SelectFormat() { id = 0, text = "Select Tenant" };
        ...
        public JsonResult SearchMobile(string q, string x)
        {
            ...
            if (!(string.IsNullOrEmpty(q) && string.IsNullOrEmpty(q)))
            {
                serialisedJson = (from b in db.Tenants
        ...
        [HttpGet]
        public JsonResult TenantCheck(string Tenant, long? cusid)
        {
            var CusCheck = db.Tenants.Where(x => x.TenantName == Tenant).Any();
            var rslt = false;
            if (CusCheck == true)
            {
                if (cusid != 0)
                {
                    var cust = db.Tenants.Where(x => x.TenantName == Tenant).FirstOrDefault();
                    CusCheck = (cust.TenantID == cusid) ? false : true;
                }
            }
        ...
        {
            Int64 ENo = 0;
            Int32 number = db.CodePrefixs.Where(a => a.section == "Tenant").Select(a => a.number).FirstOrDefault();
            if ((db.Tenants.Select(p => p.EntryNo).AsEnumerable().DefaultIfEmpty(0).Max()) == 0)
            {
                if (number == 0)
                {
                    ENo = 1;
            ...
            {
                ENo = db.Tenants.Max(p => p.EntryNo + 1);
            }
            return ENo;
        }
        ...
        {
            var prefix = db.CodePrefixs.Where(a => a.section == "Tenant").Select(a => a.prefix).FirstOrDefault();
            ...
            if (CCode == null)
            {
                ...
                {
                    CNo = db.Tenants.Max(p => p.EntryNo + 1);
                    CCode = prefix + CNo;
                    if (CodeExist(CCode))
                    {
        ...
        {
            var Exists = db.Tenants.Any(c => c.TenantCode == Code);
            bool res = (Exists) ? true : false;
            return res;
        }
