...
function addrow(t, DrCr, Account, Debit, Credit, Narration, AccName, itemdata, Bsns) {
    if (count == limits) alert("You have reached the limit of adding " + count + " inputs");
        ...
        data = "<td class='text-center'> " + slno + " </td>" +
            "<td style='display:none'><select data-name='AccType' class='form-control item_drcr_" + count + "' data-id='" + count + "' placeholder='Dr/Cr' id='drcr_" + count + "' onchange='chkdebitcredit(" + count + ")'>" + OptionCrDr + "</select></td>" +
            ...
            "<td  style='display:none'><input data-name='Credit' type='number' name='credit_rate[]' " + required + " onchange='credit_change(" + count + ");' id='price_credit_" + count + "' value='" + Credit.toFixed(2) + "' class='price_credit_" + count + " form-control text-right creditrate' placeholder='0.00' min='0' tabindex='" + tab3 + "'/></td> " +
...
function searchAccount() {
    ...
    $(".account_name").select2({
        ...
        ajax: {
            url: "/Accounts/SearchAccountsPayToexpense",
            dataType: 'json',
            delay: 50,
    ...
    $.ajax({
        ...
        success: function (e) {
            ...
            if (e.status) {
                ...
                if (url == null) {
                    window.location.href = '/JournalV/Createexpense';
                } else {
                    location.reload();
                }
            }
...
function journalVSubmit(fnval) {
    addrow('addinvoiceItem', 1, $("#ddlpayfrom").val(), 0, parseFloat($("#total_debit").text()), "", "", null, 0);
    var HTMLtbl = {
        getData: function (table) {
            var data = [];
            table.find('tr').not(':first').not('.item_').each(function (rowIndex, r) {
