...
var discountglog = 0;
var rebatelog = 0;
var readonlydiscount = "";
function addrow(t, action, ItemUnit, ItemTax, ItemTotalAmount, ItemQuantity, Item, ItemCode, ItemName, ItemUnitPrice, ItemSubTotal, ItemWithCode, ItemTaxAmount, ItemDiscount, ItemNote, itemdata, ConFactor, ItemCategory, ItemBrand, PPRICE, MRP, SellPrice, BPrice, tag1, tag2, tag3, tag4, tag5, prefix, division, supplier) {
    ...
    if (count == limits) alert("You have reached the limit of adding " + count + " inputs");
        ...
        if (readonlydiscount == "1") {
            readonlydiscount = " readonly='readonly'";
        }
        if (itemdata) {
        ...
        var itemaddbtn = "<span class='input-group-btn'><a type='button' href='/Item/Create' target=_new class=' btn btn-success btn-flat'><i class='fa fa-1x fa-plus-circle'></i></a>" + notbtn + "</span>";
        ...
        var protask = "";
        searchbtn2 = "<a type='button' href='/Item/AddItemSearch' class='modal-create-lg btn btn-success btn-flat'><i class='fa fa-1x fa-search-plus'></i></a>"
        ...
        data = "<td class='text-center' id=" + divid + "> " + slno + " </td>" +
            ...
            "<td style='width:50px;'> <input type='number' name='product_quantity[]' data-msg-min ='The Item Quantity must be Greater than Zero' onchange='quantity_change(" + count + ");' id='total_qntt_" + count + "' value='" + ItemQuantity + "'  class='total_qntt_" + count + " form-control text-right quty' placeholder='0' value='0' min='.01' tabindex='" + tab2 + "'/></td>" +
            TInRate +
            "<td><input type='number' name='product_rate[]' " + required + RateReadOnly + " data-msg-required='The Item Rate field is required' onchange='rate_change(" + count + ",\"" + type + "\");' id='price_item_" + count + "' value='" + ItemUnitPrice + "' class='price_item_" + count + " form-control text-right totrate' placeholder='0.00' min='0' tabindex='" + tab3 + "'/><input type='hidden' data-value='" + price + "' value='" + baseprice + "' name='base_rate' id='base_rate_" + count + "'> </td> " +
            ...
            "<td><input type='number' name='item_discount[]' id='item_discount" + count + "' onchange='itemdiscount_change(" + count + ",\"" + type + "\");' class='item_discount" + count + " form-control text-right item_discount' value='" + ItemDiscount + "' placeholder='0.00'" + readonlydiscount+" tabindex='" + tab3 + "'/></td>" +
        ...
        searchItemnew('#item_name_' + count );
        if (itemdata) {
        ...
        if (itemdata) {
            ...
            if (BatchEnable == "active" && itemdata.KeepStock == true && itemdata.slreq == true) {
                ...
                if (VoucherType == "Purchase" || VoucherType == "SalesReturn" || VoucherType == "Sales") {
                    // create modal
        ...
        if (ItemId != null) {
            if ($(".item_" + ItemId).length > 0) {
                if ($(".item_" + ItemId).length < 100) {
                    ...
                    alert("You Cannot Add same Item More than 100 Times");
                    $(selectObject).val(null).trigger('change');
                }
            }
            else {
...
function LastSalePurList(result, dataid) {
    var Modal = "<div id='LastPurSale-" + result.ItemID + "' class='modal fade' role='dialog' aria-hidden='true'><div class='modal-dialog modal-large'><div class='modal-content'>" +
        ...
        "<th class='text-center'>S/N</th><th class='text-center'>Date</th><th class='text-center'>BillNo</th><th class='text-center'>Supplier Name</th><th class='text-center'>Unit</th><th class='text-center'>Qty</th>" +
        "<th class='text-center'>Rate</th><th class='text-center'>Sub Total</th><th>Discount</th><th>TAX</th><th>Total</th>" +
        "</tr></thead><tbody></tbody><tfoot><tr><th></th><th></th><th></th><th></th><th></th><th class='text-right'></th><th></th><th></th><th></th><th></th></tr></tfoot></table></div>" +
...
function addLastSPRow(ItemId) {
    ...
    var purchasecount = $('#LastPurchaseCount').val() == 0 ? 20 : $('#LastPurchaseCount').val();
    ...
    if (purchasecount > 0) {
        ...
        var oTable = $('#LPurchase-' + ItemId + '').DataTable({
            ...
            "columns": [
                ...
                {
                    "name": "BillNo", "data": function (result) {



                        var urls = "";

                        urls = '<a href=\"/PurchaseEntry/Edit/' + result.PurchaseEntryId + '\" target=\"_new\">' + result.BillNo + '</a>';
...
//modal generation for purchase rack stock
...
function searchItemnew(obj) {
    var selecteditem = new Array();

    $(function searchItemnew() {
    var selecteditem = new Array();
...
function price_change(arg, type, foredit) {
    // Tax amount = Value inclusive of tax X tax rate Ã· (100 + tax rate)
    minstockcheck(arg);
    var baserate = $("#base_rate_" + arg).val();
...
function bsUpdate(selectObject, dataid) {
    $.ajax({
        ...
        success: function (result) {
            ...
            if ($(".bs_").length == 0) {
                addbillsundry('addbillsundry', 'purchase', '0.00', '', '0.00', '');
            }
...
function grandtotalcalculation() {
    ...
    $("#addbillsundry tr").each(function () {
        ...
        if (bname == "DISCOUNT" && mainflag == 0 && amt > 0 && discountglog == 0) {
            discountglog = amt;
            mainflag = 1
        }
        if (bname.indexOf("REBATE") !== -1 && amt > 0 && rebatelog == 0) {
            rebatelog = amt;
...
function bspcalculate() {
   ...
   // subtot = subtot - discountglog + parseFloat(rebatelog);
...
function bindItem(e, dvitem, type) {
    ...
    function ItemsBind(ritem, rtype, bcount) {
        ...
        if (type == "sales" || type == "nobillsundry") {
    ...
    if (Layout == "Jewellery") {
        str += '<tr id="jwltotal" class="border-top"><td colspan="2"><b>(' + (count - 1) + ' items)</b></td><td class="text-center"><b> Total Ø§Ù„Ø¬Ù…Ø§Ù„Ù‰</b></td>';
...
function bindItemBOQ(e, dvitem, type) {
    var total = parseFloat(0);
    var str = "";
    var itemcode = "";
    var count = 1;
    ...
    function ItemsBindBOQ(ritem, rtype, bcount) {
        //var itSubtotal = parseFloat(ritem.ItemSubTotal);
        // var itDiscount = parseFloat(ritem.ItemDiscount);
        // var itTaxable = itSubtotal - itDiscount;
        //var TaxableAmount = (Layout != "Scaffold") ? parseFloat(ritem.ItemSubTotal).toFixed(2) : itTaxable.toFixed(2);
...
function PrintInvoice(e, type, dvitem, conType) {
    ...
    if (typeof Bill_Tax === 'undefined') {
        Bill_Tax = "VAT(5.00%) Ø§Ù„Ø¶Ø±ÙŠØ¨Ø© ";
    }
    if (typeof Bill_Discount === 'undefined') {
        Bill_Discount = "Discount Ø®ØµÙ… ";
    }
    if (typeof Terms === 'undefined') {
        Terms = "Terms And Conditions  :";
    }
    if (typeof Bill_Amount === 'undefined') {
        Bill_Amount = "Amount ÙƒÙ…ÙŠØ© ";
    }
    ...
    if (Layout == "General") {
        ...
        if (e.summary.Discount > 0) {
            ...
            //str2 += "<td>VAT<span style='direction:ltr'>(5.00%)</span> Ø¨Ø±Ù…ÙŠÙ„ </td><td class='text-right'>" + parseFloat(e.summary.TaxAmount).toFixed(2) + "</td></tr>";
        ...
        var MpayTable = "<table class='table table-bordered' style='width:100%;border:1px solid #000'><tr class='text-center'><th rowspan='2' class='text-center'>Mode of Payment<br>Ø¹ÙØ¯Ø§ Ø©Ù‚ÙŠØ±Ø·</th><th class='text-center' rowspan='2'>Curr</th><td colspan='2'><b>Amount Ø§Ù„Ù…Ø¨Ù„Øº Ø§Ù„Ø¬Ù…Ø§Ù„Ù‰</b></td></tr><tr class='border-top text-center'><td>FC</td><td>LC</td></tr>" +
        ...
        var TotalTable = "<table class='table table-bordered' style='width:100%;border:1px solid #000'><tr class='text-center'><th class='text-center'>Total VATØ©Ø¨ÙŠØ±Ø¶Ù„Ø§ Ø¹ÙˆÙ…Ø¬Ù… </th><th class='text-right'>" + parseFloat(e.summary.TaxAmount).toFixed(2) + "</th></tr><tr><td class='text-center'>Sub Total</td><td class='text-right'>" + parseFloat(e.summary.SubTotal).toFixed(2) + "</td></tr>" + str2 +
            "<tr><td style='border-bottom: 1px solid !important;border-top: 1px solid !important;'><b>Net Total Ø©ÙŠÙ„Ø§Ù…Ø¬Ø§Ù„Ø§ Ø©Ù…ÙŠÙ‚Ù„Ø§ </b></td><td class='text-right' style='border-bottom: 1px solid !important;border-top: 1px solid !important;'><b>" + parseFloat(e.summary.GrandTotal).toFixed(2) + "</b></td></tr></table>";
        ...
        var finaltotal = '<tr class="border-top"><td colspan="2" style="border-right:0px !important;"><b> Total Taxable Amount</b> </td><td colspan="4" style="border-left:0px !important;border-right:0px !important;"><b> Ø©Ø¨ÙŠØ±Ø¶Ù„Ù„ Ø¹Ø¶Ø§Ø®Ù„Ø§ ØºÙ„Ø¨Ù…Ù„Ø§ ÙŠÙ„Ø§Ù…Ø¬Ø¥</b></td><td style="border-left:0px !important;" class="text-right"><b>' + parseFloat(e.summary.SubTotal).toFixed(2) + '</b></td></tr>' +
            '<tr class="border-top"><td colspan="2" style="border-right:0px !important;"><b>Total VAT </b></td><td colspan="4" style="border-left:0px !important;border-right:0px !important;"><b> Ø©Ø¨ÙŠØ±Ø¶Ù„Ø§ Ø¹ÙˆÙ…Ø¬Ù…</b></td><td style="border-left:0px !important;" class="text-right"><b>' + parseFloat(e.summary.TaxAmount).toFixed(2) + '</b></td></tr>' +
            '<tr class="border-top"><td colspan="2" style="border-right:0px !important;"><b>TOTAL</b></td><td colspan="4" style="border-left:0px !important;border-right:0px !important;"><b> Ø¹ÙˆÙ…Ø¬Ù…Ù„Ø§ </b></td><td style="border-left:0px !important;" class="text-right"><b>' + parseFloat(e.summary.GrandTotal).toFixed(2) + '</b></td></tr>';
        ...
        if (e.summary.Discount > 0) {
            ...
            str2 += "<tr class='border-top billsundry'><td>" + Bill_Tax + "</td><td class='billsundry text-right'>" + taxAmt + "</td></tr>";
            ...
            if ((stype == 1) || (ptype == 1)) {
                str2 += "<td class='billsundry'>" + Bill_Tax + "</td><td class='text-right billsundry'>" + taxAmt + "</td>";
            }
            //"</tr>";
        }
        if (type != "nobillsundry") {
        ...
        if (Layout == "NewDefault") {
            ...
            str3 = "<tr class='border-top billsundry'><td colspan='6' rowspan='" + count + "'><strong><u>" + Terms + "</u></strong><br/>" + tc + " </td>";
...
function PrintInvoiceBOQ(e, type, dvitem, conType) {
    ...
    var TermsActive = $("#termsactive").text();
    if (typeof Bill_Total === 'undefined') {
        Bill_Total = "Total  Ø§Ù„Ù…Ø¨Ù„Øº Ø§Ù„Ø¥Ø¬Ù…Ø§Ù„ÙŠ<span style='direction:ltr'>(AED)</span>";
    }
    if (typeof Bill_Tax === 'undefined') {
        Bill_Tax = "VAT(5.00%) Ø§Ù„Ø¶Ø±ÙŠØ¨Ø© ";
    }
    ...
    // bind items
    var itemsData = bindItemBOQ(e, dvitem, type);
    $('#itemtable tbody').html("");
    $('#itemtable').append(itemsData);
    var grt = parseFloat(e.summary.GrandTotal).toFixed(2);
    // bind total section
    ...
    if (Layout == "General") {
        ...
        if (e.summary.Discount > 0) {
            str2 += "<td>" + Bill_Discount + "</td><td id='discountprint' class='text-right'>" + disc + "</td></tr> ";
            count++;

            str2 += "<tr class='border-top'><td>" + Bill_Tax + "</td><td class='text-right'>" + taxAmt + "</td></tr>";
            ...
            var ptype = $("#PurchaseType").val();
            if ((stype == 1) || (ptype == 1)) {
                str2 += "<td>" + Bill_Tax + "</td><td class='text-right'>" + taxAmt + "</td>";
            }
            //"</tr>";
        }
        if (type != "nobillsundry") {
...
function PosSalePrint(summary, item, billsundry, trtype, type) {
    ...
    var arabic = type == "POSInvoice1" ? "" : "Ø§Ù„Ù…Ø¨Ù„Øº Ø§Ù„Ø¥Ø¬Ù…Ø§Ù„ÙŠ";
...
function GeneratePriceTable(fnval) {
    ...
    $.ajax({
        ...
        dataType: 'json',
        type: "POST",
        data: { items: items, purprices: purprice, unitpri: unitpri },
        cache: true,
        success: function (data) {
            var ittable = "";
            var promises = [];
            ...
            $.each(data.data, function (i, item) {
                var ipp = 0;
                var PricingStrategy = item.PricingStrategy;
                ipp = purprice[i];
                ...
                //promises.push($.ajax({
                //    url: '/PurchaseEntry/GetPricePurchase',
                //    type: "GET",
                //    dataType: "JSON",
                //    data: { itemid: item.ItemID, purchaseprice: ipp },
                ...
                if (PricingStrategy == true) {
                    ittable = "<tr><td>" + (i + 1) + "</td>" +
                        ...
                        "<td><input type='number' step=any data-name='SellingPrice' name='Addon[" + i + "].SellingPrice' id='SellingPrice_" + i + "' value='" + sprice + "'  class='SellingPrice_" + i + " form-control text-right sprice' data-msg-required='Selling Price is Required ' readonly/>" + "</td>" +
                        "<td><input type='number' step=any data-name='cashprice' name='Addon[" + i + "].cashprice' id='cashprice" + i + "' value='" + item.cashprice + "'  class='SellingPrice_" + i + " form-control text-right sprice' data-msg-required='Selling Price is Required ' />" + "</td>" +
                        "<td><input type='number' step=any data-name='creditprice' name='Addon[" + i + "].creditprice' id='creditprice" + i + "' value='" + item.creditprice + "'  class='SellingPrice_" + i + " form-control text-right sprice' data-msg-required='Selling Price is Required ' />" + "</td>" +

                        "<td><input type='number' step=any data-name='PurchasePrice' name='Addon[" + i + "].PurchasePrice' id='PurchasePrice_" + i + "' value='" + pprice + "'  class='PurchasePrice_" + i + " form-control text-right pprice' data-msg-required='Purchase Price is Required' readonly/>" + "</td>" +
                        "<td><input type='number' step=any data-name='BasePrice' name='Addon[" + i + "].BasePrice' id='BasePrice_" + i + "' value='" + item.BasePrice + "'  class='BasePrice_" + i + " form-control text-right bprice' data-msg-required='Base Price is Required' readonly/>" + "</td>" +
                        "<td><input type='number' step=any data-name='MRP' name='Addon[" + i + "].MRP' id='MRP_" + i + "' value='" + item.MRP + "'  class='MRP_" + i + " form-control text-right mprice' data-msg-required='MRP is Required' readonly/>" + "</td></tr>";
