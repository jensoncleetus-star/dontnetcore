...
function addrow(t, action, ItemUnit, ItemTax, ItemTotalAmount, ItemQuantity, Item, ItemCode, ItemName, ItemUnitPrice, ItemSubTotal, ItemWithCode, ItemTaxAmount, ItemDiscount, ItemNote, itemdata, ConFactor, ItemCategory, ItemBrand, PPRICE, MRP, SellPrice, BPrice, tag1, tag2, tag3, tag4, tag5, prefix, division, supplier, PricingStrategy) {
    ...
    if (count == limits) alert("You have reached the limit of adding " + count + " inputs");
        ...
        headingbtn = "<button type='button' class='itnote btn btn-default btn-flat' data-toggle='modal' data-target='#modal-item-heading" + count + "'><i class='fa fa-1x fa-file-text-o'></i></button>";
        ...
        data = "<td class='text-center' id=" + divid + "> " + slno + " </td>" +
            "<td class='input-group input-group-sm'>" + headingbtn + searchbtn2 + "<select class='form-control item_name' " + required + " data-id='" + count + "' placeholder='Item Name' id='item_name_" + count + "'  data-msg-required='The Item field is required' onchange='GetItemdetails(this," + count + ",\"" + type + "\")'>" + Option + "</select> " + itemaddbtn + "</td>" +
        ...
        //"" + setproject + settask + "";
        protask += "<td class='text-center'><button tabindex='" + tab5 + "' style='text-align: right;' class='btn btn-danger' type='button' value='Delete' onclick='deleteRow(this)'><i class='fa fa-trash fa-1x'></i></button>" + itemnote + htdata + headingitemnote + "</td>";
...
function addrowinsert(t, action, ItemUnit, ItemTax, ItemTotalAmount, ItemQuantity, Item, ItemCode, ItemName, ItemUnitPrice, ItemSubTotal, ItemWithCode, ItemTaxAmount, ItemDiscount, ItemNote, itemdata, ConFactor, ItemCategory, ItemBrand, PPRICE, MRP, SellPrice, BPrice, tag1, tag2, tag3, tag4, tag5, prefix, division, supplier, PricingStrategy) {
...
function itemUpdate2(selectObject, dataid, action) {
    ...
    $.ajax({
        ...
        type: "GET",
        dataType: "JSON",
        data: { itemID: itemid, mc: mc, Saltype: Stype, HireType: Hire },
...
function bindItem(e, dvitem, type) {
    ...
    function ItemsBind(ritem, rtype, bcount) {
        ...
        var arr = ritem.ItemNote.split('||');
        var headingexist = (arr.length >= 2 && arr[0] != "") ? true : false;
        if (headingexist == true) {
            Row = "<tr><td colspan='7'><center>" + arr[0] + "</center></td></tr>";
            itemnote = arr[1];
            count = 1;
...
function GenerateSettlementMode(fnval, SaveMode, EmpId, another = 0) {
