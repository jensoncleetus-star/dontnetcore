...
namespace QuickSoft.Areas.Property.Controllers
{
    ...
    public class DeveloperController : BaseController
    {
        ...
        //[QkAuthorize(Roles = "Dev,Developer")]
        ...
        [HttpPost]
        //[QkAuthorize(Roles = "Dev,Developer")]
        public JsonResult GetDeveloper(long? Developer, string TaxReg, long? Mobile, long? Phone, decimal? CLimit, int? CPeriod, long? Employee, string TxType, string MailId, string Alias)
        {
            ...
            var userpermission = User.IsInRole("All Developer");
            var UserId = User.Identity.GetUserId();

            var uDev = User.IsInRole("Dev");
            var uCustView = User.IsInRole("View Developer");
            var uEdit = User.IsInRole("Edit Developer");
            var uDelete = User.IsInRole("Delete Developer");
            ...
            var v = (from a in db.Developers
                     ...
                     where
                           (Developer == null || Developer == 0 || a.DeveloperID == Developer) &&
            ...
            {
                // Apply search   
                v = v.Where(p => p.DeveloperName.ToString().ToLower().Contains(search.ToLower()) ||
                                 p.DeveloperCode.ToString().ToLower().Contains(search.ToLower()) ||
            ...
            // EF Core 10 cannot translate the nested mobmodel collection-projection once Developers have rows.
            ...
            var data = page.Select(r => new {
                r.id, r.DeveloperCode, r.DeveloperName, r.TaxRegNo, r.Location, r.Address, r.Phone, r.Email,
        ...
        public ActionResult Create()
        {
            ...
            ViewBag.LastEntry = db.Developers.Select(p => p.DeveloperID).AsEnumerable().DefaultIfEmpty(0).Max();
            DeveloperViewModel vmodel = new DeveloperViewModel();
            vmodel.DeveloperCode = CustCode();
        ...
        public JsonResult Create(DeveloperViewModel vmodel)
        {
            string msg = "";
            bool stat = false;
            var custExists = db.Developers.Any(u => u.DeveloperCode == vmodel.DeveloperCode);
            if (custExists)
            {
                msg = "Developer Name already exists.";
                stat = false;
                //Danger("A Developer with same Developer Code exists.", true);
            ...
            {               
                ...
                {
                    //var bro = db.companys.Select(a => a.Developer).FirstOrDefault();
                    //if (bro == null)
                    //{
                    //    msg = "Please give Account Group for Developer in Company.";
                    //    stat = false;
                    ...
                    account.Name = vmodel.DeveloperName;
                    account.Alias = vmodel.Alias;
                    account.PrintName = vmodel.DeveloperName;
                    ...
                    if (vmodel.DeveloperID != 0 && vmodel.DeveloperID != null)
                    {
                        Developer cus = db.Developers.Find(vmodel.DeveloperID);
                        cus.Accounts = account.AccountsID;
                        cus.DeveloperName = vmodel.DeveloperName;
                        cus.DeveloperCode = vmodel.DeveloperCode;
                        ...
                        //cus.Type = CRMDeveloperType.Developer;


                        db.Entry(cus).State = EntityState.Modified;
                        db.SaveChanges();
                        CustId = cus.DeveloperID;

                        Contact cont = db.Contacts.Find(cus.Contact);

                        cont.Name = vmodel.DeveloperName;
                    ...
                    else
                    {
                        ...
                        Developer cus = new Developer
                        {
                            EntryNo = GetEntryNo(),
                            Contact = contactId,
                            Accounts = accountId,
                            DeveloperName = vmodel.DeveloperName,
                            DeveloperCode = vmodel.DeveloperCode,
                            ...
                            Swift = vmodel.Swift,
                            Type = vmodel.Type,
                            //Type = CRMDeveloperType.Developer,
                            //SourceOfLead = vmodel.SourceOfLead,
                        };
                        db.Developers.Add(cus);
                        db.SaveChanges();
                        CustId = cus.DeveloperID;
                        ...
                        foreach (var arr in vmodel.docmodel)
                        {
                            if (arr.Date != null)
                            {
                                ...
                                if (arr.Attachments != null)
                                {
                                    if (arr.Attachments != null)
                                    {
                                        string storePath = LegacyWeb.MapPath("~/uploads/TenancyContractDocument/" + "Developer_" + docid);
                                        ...
                                        var uploadUrl = LegacyWeb.MapPath("~/uploads/TenancyContractDocument/" + "Developer_" + docid + "/");
                    ...
                    com.addlog(LogTypes.Created, UserId, "Developer", "Developer", findip(), CustId, "Developer Added Successfully");

                    msg = "Successfully added Developer details.";
                    stat = true;
                    return new QuickSoft.Models.LegacyJsonResult { Data = new { status = stat, message = msg } };
                }
                else
        ...
        public Boolean InsertToSales(ReferenceAccountViewModel arr, long CustId, string UserId, long branch)
        {
            ...
            SEentry.Remarks = "Sales Entry From Developer Creation";
        ...
        public ActionResult Edit(long? id)
        {
            ...
            var cus = (from a in db.Developers
                       join b in db.Accountss on a.Accounts equals b.AccountsID
                       where a.DeveloperID == id && (userpermission == true || b.CreatedBy == b.CreatedBy)
                       select new
                       {
                           a.Contact,
                           a.Accounts,
            ...
            DeveloperSubmitViewModel landmodel = new DeveloperSubmitViewModel();
            landmodel.DeveloperID = cus.DeveloperID;
            landmodel.DeveloperName = cus.DeveloperName;
            landmodel.DeveloperCode = cus.DeveloperCode;
            ...
            var cust = db.Developers.Select(r => new
            {
                ID = r.DeveloperName,
            ...
            ViewBag.preEntry = db.Developers.Where(a => a.DeveloperID < id && (userpermission == true)).Select(a => a.DeveloperID).DefaultIfEmpty().Max();
            ViewBag.nxtEntry = db.Developers.Where(a => a.DeveloperID > id && (userpermission == true)).Select(a => a.DeveloperID).DefaultIfEmpty().Min();
        ...
        [HttpPost]
        //[QkAuthorize(Roles = "Dev, Create Developer")]
        public JsonResult Update(DeveloperSubmitViewModel cusmodel)
        {
            bool stat = false;
            string msg;
            var UserId = User.Identity.GetUserId();
            var Branch = db.Users.Where(a => a.Id == UserId).Select(a => a.BranchID).FirstOrDefault();
            var id = cusmodel.DeveloperID;
            if (ModelState.IsValid)
            {
                var CodeExists = db.Developers.Any(u => u.DeveloperCode == cusmodel.DeveloperCode && u.DeveloperID != id);
                if (CodeExists)
                {
                    msg = "A Developer with same Developer Code exists.";
                    stat = false;
                }
                else
                {
                    Developer cus = db.Developers.Find(id);
                    cus.DeveloperName = cusmodel.DeveloperName;
                    cus.DeveloperCode = cusmodel.DeveloperCode;
                    ...
                    db.Entry(cus).State = EntityState.Modified;
                    db.SaveChanges();
                    Int64 CustId = cus.DeveloperID;

                    Contact cont = db.Contacts.Find(cus.Contact);

                    cont.Name = cusmodel.DeveloperName;
                    ...
                    Accounts account = db.Accountss.Find(cus.Accounts);
                    account.PrintName = cusmodel.DeveloperName;
                    account.Name = cusmodel.DeveloperName;
                    account.Alias = cusmodel.Alias;
                    account.TRN = cusmodel.TaxRegNo;
                    account.Group = Convert.ToInt64(cusmodel.AccountGroup);
                    ...
                    var bro = db.companys.Select(a => a.Developer).FirstOrDefault(); //db.EnableSettings.Where(a => a.EnableType == "Developer").Select(a => a.TypeValue).FirstOrDefault();
                    ...
                    //              where a.DeveloperId == id && a.Recieptid == 0
                    ...
                    //    //delete other sales
                    //    var salelist = db.SalesEntrys.Where(a => a.Developer == CustId && a.Status == 0).ToList();
                    ...
                    foreach (var arr in cusmodel.docmodel)
                    {
                        ...
                        if (Convert.ToInt32(arr.ID) == 0 && arr.Type != null)
                        {
                            ...
                            doc.DocumentType = Convert.ToInt64(arr.Type);
                            doc.Purpose = "Developer";
                            doc.Reference = cusmodel.DeveloperID;
                            if (arr.Date != null)
                                doc.ExpDate = DateTime.Parse(arr.Date, new CultureInfo("en-GB"));
                            else
                                doc.ExpDate = null;
                    ...
                    com.addlog(LogTypes.Updated, UserId, "Developer", "Developers", findip(), cus.DeveloperID, "Developer Updated Successfully");

                    msg = "Successfully updated Developer details.";
                    stat = true;
                }
            }
            else
        ...
        [HttpPost]
        //[QkAuthorize(Roles = "Dev,Delete Developer")]
        public ActionResult DeleteAllDeveloper(long[] bill)
        {
            Int32 count = 0;
            Int32 notdel = 0;
            ...
            if (count > 0 && notdel > 0)
            {
                Success("Deleted " + count + " Developers, Unable to Delete " + notdel + " Developers. ", true);
            }
            else if (notdel > 0)
            {
                Danger("Unable to Delete " + notdel + " Developers.", true);
            }
            else
            {
                Success("Deleted " + count + " Developers.", true);
            }
        ...
        [RedirectingAction]
        //[Authorize(Roles = "Dev,Delete Developer")]
        public ActionResult Delete(long? id)
        {
            ...
            Developer ptype = db.Developers.Find(id);
            if (ptype == null)
            {
                return NotFound();
            }
        ...
        public ActionResult DeleteConfirmed(long id)
        {
            ...
            {
                stat = DeleteFn(id);
                msg = "Successfully Deleted Developer details.";
            }
            return new QuickSoft.Models.LegacyJsonResult { Data = new { status = stat, message = msg } };
        }
        ...
        {
            var UserId = User.Identity.GetUserId();
            db.PropertyDocumentTypes.RemoveRange(db.PropertyDocumentTypes.Where(a => a.Reference == id && a.Purpose == "Developer"));
            db.SaveChanges();
            Developer pt = db.Developers.Find(id);

            db.Developers.Remove(pt);
            db.SaveChanges();

            com.addlog(LogTypes.Deleted, UserId, "Developer", "Developer", findip(), pt.DeveloperID, "Developer Deleted Successfully");
            return true;
        }
        ...
        [HttpGet]
        //[QkAuthorize(Roles = "Dev,View Developer")]
        public ActionResult Details(int? id)
        {
            ...
            DeveloperSubmitViewModel cusmodel = new DeveloperSubmitViewModel();
            cusmodel = (from a in db.Developers
                        join b in db.Contacts on a.Contact equals b.ContactID into tmp
                        from b in tmp.DefaultIfEmpty()
                        join e in db.Accountss on a.Accounts equals e.AccountsID
                        where a.DeveloperID == id
                        select new DeveloperSubmitViewModel
                        {
            ...
            if (cusmodel != null)
            {
                var devContact = db.Developers.Where(d => d.DeveloperID == id).Select(d => d.Contact).FirstOrDefault();
                cusmodel.mobmodel = db.Mobiles.Where(ac => ac.Contact == devContact)
        ...
        public JsonResult GetMobile(long CnId)
        {
            var ConD = (from a in db.Mobiles
                        join b in db.Developers on a.Contact equals b.Contact
                        where b.DeveloperID == CnId
                        select new
                        {
                            Mob = a.MobileNum,
                            Name = a.Name
        ...
        public JsonResult SearchDeveloper(string q, string x)
        {
            List<SelectFormat> serialisedJson;
            if (!(string.IsNullOrEmpty(q) && string.IsNullOrEmpty(q)))
            {
                serialisedJson = (from a in db.Developers
                                  where a.DeveloperName.ToLower().Contains(q.ToLower()) || a.DeveloperName.Contains(q)
                                  select new SelectFormat
                                  {
                                      text = a.DeveloperName,
                                      id = a.DeveloperID
                                  })
            ...
            {
                serialisedJson = db.Developers.Select(b => new SelectFormat
                {
                    text = b.DeveloperName,
                    id = b.DeveloperID
                }).OrderBy(b => b.text).ToList();
            ...
            {
                var initial = new SelectFormat() { id = 0, text = "Select Developer" };
        ...
        public JsonResult SearchMobile(string q, string x)
        {
            ...
            if (!(string.IsNullOrEmpty(q) && string.IsNullOrEmpty(q)))
            {
                serialisedJson = (from b in db.Developers
        ...
        [HttpGet]
        public JsonResult DeveloperCheck(string Developer, long? cusid)
        {
            var CusCheck = db.Developers.Where(x => x.DeveloperName == Developer).Any();
            var rslt = false;
            if (CusCheck == true)
            {
                if (cusid != 0)
                {
                    var cust = db.Developers.Where(x => x.DeveloperName == Developer).FirstOrDefault();
                    CusCheck = (cust.DeveloperID == cusid) ? false : true;
                }
            }
        ...
        {
            Int64 ENo = 0;
            Int32 number = db.CodePrefixs.Where(a => a.section == "Developer").Select(a => a.number).FirstOrDefault();
            if ((db.Developers.Select(p => p.EntryNo).AsEnumerable().DefaultIfEmpty(0).Max()) == 0)
            {
                if (number == 0)
                {
                    ENo = 1;
            ...
            {
                ENo = db.Developers.Max(p => p.EntryNo + 1);
            }
            return ENo;
        }
        ...
        {
            var prefix = db.CodePrefixs.Where(a => a.section == "Developer").Select(a => a.prefix).FirstOrDefault();
            ...
            if (CCode == null)
            {
                ...
                {
                    CNo = db.Developers.Max(p => p.EntryNo + 1);
                    CCode = prefix + CNo;
                    if (CodeExist(CCode))
                    {
        ...
        {
            var Exists = db.Developers.Any(c => c.DeveloperCode == Code);
            bool res = (Exists) ? true : false;
            return res;
        }
