...
(sourcelead == 0 || sourcelead == null | (leads != null && leads.SourceOfLead == sourcelead))
        ...
        public decimal GetItemPurchasePrice(long? ItemId, DateTime? SEDate, long? mc, long? salesentrydetailid, bool moment = false, decimal? reqqty = 0)
        {
            ...
            if (items == null) return 0; // guard: item may be deleted while it still appears on an old invoice line
            ...
            if (confactor == 0) confactor = 1;       // guard: ConFactor defaults to 0 -> divide-by-zero below
            if (confactoract == 0) confactoract = 1;
