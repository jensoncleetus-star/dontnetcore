...
namespace QuickSoft.Areas.Property.Controllers
{
    ...
    public class RentalController : BaseController
    {
        ...
        // Custom branded, print-ready Rental Invoice document (standalone page -> browser print / PDF).
        // Read-only; every piece materialized before shaping (EF Core 10 safe).
        [HttpGet]
        public ActionResult Print(long id)
        {
            var r = db.Rentals.Where(x => x.RentalID == id)
                .Select(x => new
                {
                    x.RentalID, x.VoucherNo, x.RDate, x.Tenant, x.Property, x.Unit,
            ...
            var fields = (from a in db.AdditionalFieldDatas
                          where a.Reference == r.RentalID && a.Purpose == "Rental"
            ...
            ViewBag.Id = r.RentalID;
            ViewBag.Code = string.IsNullOrEmpty(r.VoucherNo) ? ("RNT-" + r.RentalID) : r.VoucherNo;
            ...
            ViewBag.InvoiceDate = r.RDate.ToString("dd MMM yyyy");
