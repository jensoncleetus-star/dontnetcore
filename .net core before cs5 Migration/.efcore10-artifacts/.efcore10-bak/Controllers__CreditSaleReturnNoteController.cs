...
{
    [RedirectingAction]
    public class CreditSaleReturnNoteController : BaseController
    {
        ApplicationDbContext db;
        Common com;
        public CreditSaleReturnNoteController()
        {
        ...
        public ActionResult GetSalesReturn(string BillNo, string FromDate, string ToDate, long? customer, long? salesperson, long? type, string user, int? Balance, long? MC, string appstat, long? ProjectName, long? Task)
        {
            ...
            var serverQuery = (from a in db.SalesReturns
                    ...
                    from kk in crsales.DefaultIfEmpty()
                     ...
                     select new
                     {
                         validornot = tem != 1 && (EF.Functions.DateDiffDay(a.SRDate, editableDay) <= 0 && EF.Functions.DateDiffDay(a.SRDate, today) >= 0) ? "valid" : "invalid",
        ...
        public ActionResult Createonly(long? id, string type)
        {
            ...
            if (id != null)
            {
                if (type == "Sales")
                {
                    var b = db.SalesEntrys.Find(id);
                    decimal percid = 0;
                    if (b.pricecategoryid != null)
                    {
                        percid = db.PriceCategories.Find(b.pricecategoryid).value;
                    }
                    salesentry = new SalesReturnViewModel
                    {
                        SRDate = b.SEDate,
                        pricecategoryper = percid,
                        BillNo = InvoiceNo(),
                        SalesEntryId = b.SalesEntryId,
                        SRCashier = b.SECashier,
                        Customer = b.Customer,
                        SRDiscount = b.SEDiscount,
                        SRGrandTotal = b.SEGrandTotal,
        ...
        [HttpPost]
        [QkAuthorize(Roles = "Dev,Sales Return Entry")]
        public JsonResult CreateSalesReturn(string[][] array, string[] saledata, SRBillSundryViewModel bsmodel, ICollection<BatchStockPViewModel> bstmodel,long? discountrate)
        {
            bool stat = false;
            string msg;
            ...
            if ((sr.SEDate - editableDay).TotalDays < 0 || tem == 1)
            {
            ...
            if (!BillExist(Convert.ToString(saledata[18])))
            {
             ...
             UserId = User.Identity.GetUserId();
                ...
                {
                    var discvalue = Convert.ToDecimal(arr[6]);
                    if (discountrate==1)
                    {
                        discvalue = discvalue * Convert.ToDecimal(arr[2]);
                    }
                ...
                //// execute sp sql 
                string sql = String.Format("EXEC {0} {1};", "SP_InsertSRNoteItems", "@TableType");
                //// execute sql 
        ...
        [HttpGet]
        [QkAuthorize(Roles = "Dev,View Sales Return")]
        public ActionResult Details(long? id)
        {
            ...
            vmodel.SRItemss = db.SRNoteItemss.Where(a => a.SalesReturnId == id && a.itemNote != "-:{Bundle_Item}")
            .Select(b => new SRItemViewModel
            {
                ...
                bundleitem = (from ab in db.SRNoteItemss
       ...
       .Select(b => new SRBillSundryViewModel
       {
        ...
        public ActionResult Edit(long? id)
        {
            ...
            if ((Salertn.SRDate - editableDay).TotalDays < 0 || tem == 1)
            {
                return NotFound();
            ...
            vmodel = (from b in db.SalesReturns
                      ...
                      let prid= db.SalesEntrys.Where(o => o.SalesEntryId == b.SalesEntryId).Select(o => o.pricecategoryid).FirstOrDefault()
                      where b.SalesReturnId == id
                      select new SalesReturnViewModel
                      {
                          SRDate = b.SRDate,
                          BillNo = b.BillNo,
                          SalesEntryId = b.SalesEntryId,
                          pricecategoryper=(prid!=null)?db.PriceCategories.Where(o=>o.pricestratagyid==prid).Select(o=>o.value).FirstOrDefault():0,
        ...
        [HttpPost]
        [QkAuthorize(Roles = "Dev,Edit Sales Return")]
        public JsonResult UpdateSalesReturn(string[][] array, string[] saledata, SRBillSundryViewModel bsmodel, ICollection<BatchStockPViewModel> bstmodel)
        {
            bool stat = false;
            string msg;
            ...
            if (com.chkApproved(salesReturnId, EditPermission, "SalesReturn", UserId) == true)
            {
                ...
                var SRItem = db.SRNoteItemss.Where(a => a.SalesReturnId == salesReturnId).FirstOrDefault();
                if (SRItem != null)
                {
                    var SItems = db.SRNoteItemss.Where(a => a.SalesReturnId == salesReturnId).ToList();
                    ...
                    db.SRNoteItemss.RemoveRange(db.SRNoteItemss.Where(a => a.SalesReturnId == salesReturnId));
                    db.SaveChanges();
        ...
        [HttpGet]
        [QkAuthorize(Roles = "Dev,Download Sales Return")]
        ...
        public StringBuilder generatePdf(long salesRetId)
        {
            ...
            var details = (from a in db.SalesReturns
                           ...
                           select new
                           {
                               ...
                               Paid = d.SReturnAmount,
                               Balance = a.SRGrandTotal - d.SReturnAmount,
            ...
            var saleitem = (from b in db.SRNoteItemss
        ...
        {
            var ConD = (from a in db.SRNoteItemss
                        ...
                        select new
                        {
                            ...
                            batch = (from ay in db.BatchStocks
                                     join az in db.SRNoteItemss on new { f1 = ay.Item, f2 = ay.Unit, f3 = ay.Reference, f4 = ay.Type }
      ...
      //  [ValidateAntiForgeryToken]
        ...
        public bool DeleteFn(long sId)
        {
            ...
            var SRItem = db.SRNoteItemss.Where(a => a.SRItemsId == sId);
            ...
            if (SRItem != null)
            {
                ...
                db.SRNoteItemss.RemoveRange(db.SRNoteItemss.Where(a => a.SalesReturnId == sId));
                db.SaveChanges();
            }
            if (SRBs != null)
            {
        ...
        {
            var companyPrefix = db.CodePrefixs.Where(a => a.section == "SalsReturnNote").Select(a => a.prefix).FirstOrDefault();
            Int32 number = db.CodePrefixs.Where(a => a.section == "SalsReturnNote").Select(a => a.number).FirstOrDefault();

            if (billNo == null)
            {
                if ((db.SalesReturns.Where(o=>o.Ref5== "Credit Note").Select(p => p.SRNo).AsEnumerable().DefaultIfEmpty(0).Max()) == 0)
                {
                    billNo = (number == 0) ? (companyPrefix + 1) : (companyPrefix + number);
                }
                else
                {
                    SRNo = db.SalesReturns.Where(o => o.Ref5 == "Credit Note").Max(p => p.SRNo + 1);
        ...
        {
            var Exists = db.SalesReturns.Where(o => o.Ref5 == "Credit Note").Any(c => c.BillNo == SRNo);
        ...
        private long GetSRNo()
        {
            ...
            {
                PNo = db.SalesReturns.Where(o => o.Ref5 == "Credit Note").Max(p => p.SRNo + 1);
            }
            return PNo;
        }
        ...
        public JsonResult GetMaxReturnId(long? SalesEntryId, string SaveMode)
        {
            ...
            if (SaveMode == "Edit")
            {
                var RetnDtls = db.SalesReturns.Where(o => o.Ref5 == "Credit Note").Where(a => a.SalesEntryId == SalesEntryId).FirstOrDefault();

                if (RetnDtls != null)
                    return Json(db.SalesReturns.Where(o => o.Ref5 == "Credit Note").Where(a => a.SalesEntryId == SalesEntryId).Select(a => a.SRNo));
                else
