...
function addrow(t, action, Barcode, UnitId, Quantity, Price, Amount, DepreciationPercentage, AssetAccountId, DepreciationAccountId, Item, ItemName, ItemWithCode, Name, PrintName, ItemUnitName, ItemNote, itemdata) {
    if (count == limits) alert("You have reached the limit of adding " + count + " inputs");
    else {
        var Option = "";
        var AssetOption = "";
        var DepreciationOption = "";
        var optionunit = "";
        var required = "";
        var slno = $('#addinvoiceItem tr').length + 1;
        var a = "item_name" + count,
            tabindex = count * 5;
        var row = "<tr class='item_" + Item + "'id='item_" + count + "'>";
        var data = "";
        var price = 0;
        var baseprice = 0;
        var mrp = 0;
        ...
        //**...lineno:36 to 57 for load the item,asset,depreciation in edit mode....**//
        ...
        if (Item != null) {
            row = "<tr class='item_" + UnitId + "' id='item_" + count + "'>";
            optionunit = "<option value='" + UnitId + "'>" + ItemUnitName + "</option>";

        }
        if (Item != null) {
            row = "<tr class='item_" + AssetAccountId + "' id='item_" + count + "'>";
            AssetOption = "<option value='" + AssetAccountId + "'>" + Name + "</option>";
        }
        if (Item != null) {

            row = "<tr class='item_" + DepreciationAccountId + "' id='item_" + count + "'>";
            DepreciationOption = "<option value='" + DepreciationAccountId + "'>" + PrintName + "</option>";
        }
        ...
        data = "<td class='text-center' id=" + divid + "> " + slno + " </td>" +
            "<td class='input-group input-group-sm'><select class='form-control item_name' " + required + " data-id='" + count + "' placeholder='Item Name' data-msg-required='The Item Name is required' id='item_name_" + count + "'  data-val-required='The Item field is required' onchange='GetItemdetails(this," + count + ",\"" + type + "\")'>" + Option + "</select></td>" +

            "<td><input type='text' style='width:140px;' name='Barcode' id='Barcode_" + count + "'  class='form-control'  value='" + Barcode + "'</td>" +
            "<td style='width:100px;'><select class='form-control units unit_name_" + count + "' id='unit_name_" + count + "' " + required + " data-id='" + count + "' id='unit_name' onchange='unitchange(this," + count + ",\"" + type + "\");'>" + optionunit + "</select></td>" +
            "<td><input type='number'  data-msg-required='The Quantity is required' name='product_quantity[]'" + " onchange='quantity_change(" + count + ");' id='total_qntt_" + count + "' value='" + Quantity + "'  class='total_qntt_" + count + " form-control text-right quty' placeholder='0.00' min='0' tabindex='" + tab2 + "'/></td>" +
            "<td><input type='number' data-msg-required='The Price is required' name='product_rate[]' "  + " onchange='rate_change(" + count + ",\"" + type + "\");' id='price_item_" + count + "' value='" + Price + "' class='price_item_" + count + " form-control text-right totrate' placeholder='0.00' min='0' tabindex='" + tab3 + "'/><input type='hidden' data-value='" + price + "' value='" + baseprice + "' name='base_rate' id='base_rate_" + count + "'></td>" +
            "<td><input type='number' name='sub_total[]' id='sub_total_" + count + "' "  + "class='sub_total_" + count + " form-control text-right subtotal' value='" + Amount + "'   id='depreciation_per" + count + "'data-val-required='The Depreciation Percentage field is required'  placeholder='0.00' min='0' tabindex='" + tab3 + "' readonly='readonly'/><input type='hidden' class='cfactor' name='cfactor' id='cfactor_" + count + "'/>" +
            "<td><input type='text' name='Depreciation_Percentage[]' id='Percentage_" + count + "' " + "class='form-control text-center' value='" + DepreciationPercentage + "' id='depreciation_name" + count + "'data-val-required='The Depreciation Percentage field is required'  placeholder='0'</td>" +
            "<td><select  class='form-control account_name' name='Asset_" + count + "'" + " data-id='" + count + "'placeholder='Account Name' id='account_name" + count + "'data-val-required='The Account field is required' >" + AssetOption + " style='width:150px;'</select></td>" +
            "<td><select class='form-control account_depreciation' name='Depreciation_" + count + "'" + " data-id='" + count + "'placeholder='Account_Depreciation'  id='account_depreciation" + count + "'data-val-required='The Account Depreciation is required'  >" + DepreciationOption + "</select></td>" +
    ...
    $.ajax({
        ...
        success: function (result) {
            if (action != "sales" || (action == "sales" && result.KeepStock != true) || (action == "sales" && result.KeepStock == true && result.total > 0)) {
                ...
                //}
                //if (action == "purchase") {
                $("#Barcode_" + dataid).val(result.Barcode);
                //var a = $("Barcode_").val();
                ...
                if (res == true) {
                    ...
                    if ($(".item_").length == 0) {
                        addrow('addinvoiceItem', "", "", '0', '0.00', '0', "0", "0", "", "", "", "", "", "", "", "", "", "");
                    }
                    $('.unit_name_' + dataid).focus();
                }
                else {
...
function createBarcode(result, dataid) {
    var ROnlyRate = $("#ROnlyRate").val();
    if (ROnlyRate == "active") {
        $("#Barcode_" + dataid).attr('readonly', true);
    }
    // clear previous content
    $('#Barcode_' + dataid).empty();
...
function searchDepreciation() {
    ...
    $(".account_depreciation").select2({
        placeholder: 'Search Depreciation Account ',
        minimumInputLength: 0,
        ajax: {
            url: "/AssetFromInventory/SearchDepreciation",
            dataType: 'json',
            type: "POST",
            delay: 50,
            data: function (params) {
