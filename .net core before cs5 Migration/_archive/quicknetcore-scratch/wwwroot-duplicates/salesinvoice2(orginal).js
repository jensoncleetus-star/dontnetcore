...
function minstockcheck(arg, action) {
    ...
    var edit = $('.box-title').text().indexOf('Edit SalesEntry-Invoice');
    if (edit >= 0) {
        action = 'foredit';
    }
    if (keepstock == "yes") {
...
function repoFormatResult(repo) {
    ...
    markup += "<a  style='color:red;' id='showdesc' > <div style='' class='showdesc'>" + repo.ItemDescription + "</div></a>";
...
function bindItem(e, dvitem, type) {
    ...
    function ItemsBind(ritem, rtype, bcount, hideItemName) {
        ...
        if (dvitem != "active" && rtype != "bundle") {
            ...
            dvField2 += '<td class="text-righ sub_totalt" style="display:none;">' + bold + parseFloat(ritem.ItemTaxAmount).toFixed(2) + boldend + '</td>';
            dvField2 += '<td class="text-right sub_total">' + bold + parseFloat(ritem.ItemUnitPrice * ritem.ItemQuantity).toFixed(2) + boldend + '</td>';
