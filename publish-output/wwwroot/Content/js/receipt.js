    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.status == false) {
                ...
                if (data.status) {
                    if (data.type == 'print') {
                        $("#lblcreditsum").text(parseFloat(data.GrandTotal - data.Discount).toFixed(2)); (data.data, data.tbldata, data.fmapp);
                    }
                    else {
                        $('.ajax_response', res_success).text(data.message);
                        $('.AlertDiv').prepend(res_success);
...
function recprint(data, tbldata, fmapp) {
    ...
    if (data.Discount == 0) {
        ...
        $("#lblcreditsum").text(parseFloat(data.GrandTotal - data.Discount).toFixed(2));
        $('#discountid').show();
    }
    ...
    var grt = parseFloat(data.GrandTotal - data.Discount).toFixed(2);
