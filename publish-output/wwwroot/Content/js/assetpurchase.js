...
function addrow(count, t, action, AssetName, Barcode, UnitId, Quantity, Price, TotalPrice, DepreciationPercentage, AssetAccountId, DepreciationAccountId, AccName, DccName, Uni, Deppreciation, Item, ItemName, ItemWithCode, ItemNote, itemdata, Item) {
    count = count + 1;

    if (count == limits) alert("You have reached the limit of adding " + count + " inputs");
    else {
        jQuery(function () {
            jQuery(".account_name").click();
            jQuery(".account_depreciation").click();
            jQuery(".unit_name_").click();
            jQuery(".Barcode").click();
        });
        var Option = "";
        var Option1 = "";
        var Option2 = "";
        ...
        if (AssetAccountId != null) {
            row = "<tr class='account_name" + AssetAccountId + "' id='item_" + count + "'>";
            Option1 = "<option value='" + AssetAccountId + "'>" + AccName + "</option>";
        }
        if (DepreciationAccountId != null) {
            row = "<tr class='account_depreciation" + DepreciationAccountId + "' id='item_" + count + "'>";
            Option2 = "<option value='" + DepreciationAccountId + "'>" + DccName + "</option>";

        }
        if (Uni != null) {
            row = "<tr class='unit_name_" + UnitId + "' id='item_" + count + "'>";
            Option3 = "<option value='" + UnitId + "'>" + Uni + "</option>";
        }
        if (count == 0) {
        ...
        data = "<td class='text-center' id=" + divid + "> " + slno + " </td>" +
            "<td class='input-group input-group-sm' ><input type='text' class='form-control  item_name item_' " + required + " data-id='" + count + "' placeholder='Item Name' style='width:20rem;' value='" + AssetName + "'  id='item_name_" + count + "'  onchange='GetItemdetails(this," + count + ",\"" + type + "\")'>" + "</td>" +
            "<td ><input type='text' class='form-control Barcode text-center' " + " data-id='" + count + "' placeholder='Barcode' value='" + Barcode + "' style='width:20rem;'  id='Barcode" + count + " ' readonly='readonly'></td>" +
            "<td><select class='form-control  unit_name_ ' name ='Unit_" + count + "' id='unit_name_" + count + "' " + required + " data-id='" + count + "' onclick='searchUnit()'>'" + Option3 + "'</select></td>" +
            "<td><input type='number' name='product_quantity[]' onchange='quantity_change(" + count + ");' id='total_qntt_" + count + "' value='" + Quantity + "'  class='text-center total_qntt_" + count + " form-control text-right quty' placeholder='Enter Quantity' min='0' tabindex='" + tab2 + "'/></td>" +
            "<td><input type='number' data-msg-required='The Item Rate is required' name='product_rate[]' " + required + " onchange='rate_change(" + count + ",\"" + type + "\");' id='price_item_" + count + "' value='" + Price + "' class='text-center price_item_" + count + " form-control text-right totrate' placeholder='0.00' min='0' tabindex='" + tab3 + "'/><input type='hidden' data-value='" + price + "' value='" + baseprice + "' name='base_rate' id='base_rate_" + count + "'></td>" +
            "<td><input type='number' name='sub_total[]' id='sub_total_" + count + "' class='text-center sub_total_" + count + " form-control text-right subtotal' value='" + TotalPrice + "'   placeholder='0.00' min='0' tabindex='" + tab3 + "' readonly='readonly'/><input type='hidden' class='cfactor' name='cfactor' id='cfactor_" + count + "'/>" +
            "<td style='width:100px;' class='input-group'><input type='number' name='Deppercentage' class='text-center form-control text-right' " + required + " data-id='" + count + "' value='" + DepreciationPercentage + "' min='0'   id='Deppreciation" + count + "'   >" + "</td>" +
            "<td><select class='form-control account_name' name='Asset_" + count + "'" + required + " data-id='" + count + "'placeholder='Account Name'data-msg-required='The Account Name is required' id='account_name" + count + "'data-val-required='The Account field is required'  onclick='searchAccount()'> " + Option1 + " </select></td>" +
            "<td><select class='form-control account_depreciation' name='Depreciation_" + count + "'" + required + " data-id='" + count + "'placeholder='Account_Depreciation' data-msg-required='The Depreciation Account is required' id='account_depreciation" + count + "'data-val-required='The Account Depreciation is required'  onclick='searchDepreciation()'>" + Option2 + "</select></td>" +

            "<td class='text-center'><button tabindex='" + tab5 + "' style='text-align: right; margin-top: 7px;' class='btn btn-danger dlt ' type='button'  value='Delete' onclick='deleteRow(this)'><i class='fa fa-trash fa-1x ' ></i></button>" + itemnote + htdata + "</td>";
        row += data + "</tr>";
        $('#' + t).append(row);
...
function GetItemdetails(selectObject, dataid, action) {
    ...
    var counts = $('.item_name').filter(function (input) {
        return $(this).val() == '';
    }).length;
    if (counts == 0) {
        addrow(count, 'addinvoiceItem', '', '', '', '0.00', '0.00', '0.00', '0.00', '0');
    }
}
// update item details
...
function searchUnit() {
    var selectedaccount = new Array();
    $(".unit_name_").each(function () {
...
function searchDepreciation() {
    var selectedaccount = new Array();
    $(".account_depreciation").each(function () {
        selectedaccount.push($(this).val());
    });

    $(".account_depreciation").select2({
        placeholder: 'Select',
        minimumInputLength: 0,
        ajax: {
...
function CalculatetblItemListSum() {
    ...
    if (tbody.children().length > 0) {
        ...
        $("#SubTotal").text((gtSubTotal));
        $("[id$=ItemDisc]").text(parseFloat(gtDiscount).toFixed(2));
        $(".TotalAssetValue").val(gtSubTotal);
