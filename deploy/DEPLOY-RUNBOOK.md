...
2. Run the idempotent upgrade scripts against the restored DB **in order** (minutes total):
   ...
   - **(OPTIONAL — only branches that use the Real Estate / Property module)**
     [`Sql/PropertySchema_Ensure.sql`](../Sql/PropertySchema_Ensure.sql) — idempotent: ensures all 33 Property tables + columns exist exactly as the canonical schema (incl. `PropertyMains.LandlordID` the EF model needs). A no-op on a DB that already has them (the live branch DBs do, since these tables ship with the product); include it only as a safety net or for a DB that predates the module. Then
     [`Sql/AddRealEstateMenu.sql`](../Sql/AddRealEstateMenu.sql) — inserts the 37 `AppModules` rows for the **Real Estate** top menu (Master/Transactions/Reports/Settings → the `/Property/*` pages). Menu is **per-user opt-in** (default OFF): after running, enable "Real Estate" for each user in User management → re-login. Rollback: `DeleteRealEstateMenu.sql`. Skip both entirely if the branch does not use Real Estate.
